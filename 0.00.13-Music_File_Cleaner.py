...
class FileDisplay:
    ...
    def _draw_discogs_info_column(self, screen):
        ...
        # Title
        title_label = self.font_label.render("Discogs Title:", True, (200, 200, 200))
        screen.blit(title_label, (475, y_offset))
        self._draw_match_indicator(450, y_offset + 5, self.title_match, self.title_case_mismatch)
        y_offset += 30
        title_surf = self.font_data.render(self.discogs_title, True, (255, 255, 255))
        screen.blit(title_surf, (475, y_offset))
        y_offset += 50

        # Year, Genre, Length
        year_surf = self.font_data.render(f"Year: {self.year}", True, (255, 255, 255))
        screen.blit(year_surf, (475, y_offset))
        y_offset += 30
...
def main():
    ...
    while not done:
        ...
        for event in pygame.event.get():
            ...
            if app_state == "file_display" and file_display:
                ...
                if action == "continue":
                    ...
                    file_display = None
                    app_state = "searching_music_file" # Go to search screen for next file
        ...
        if app_state == "searching_music_file":
            # If a search isn't running, start one.
            if search_thread is None:
                if current_file_index >= len(music_files):
                    print("Finished all files.")
                    done = True
                else:
                    # Define a worker function to run in the background
                    def search_worker(screen_ref, music_file):
                        try:
                        ...
                        result = FileDisplay(screen_ref, music_file, artist, title)
                        search_result['file_display'] = result

                    current_file = music_files[current_file_index]
                    search_result.clear()
                    search_thread = threading.Thread(target=search_worker, args=(screen, current_file))
                    search_thread.start()
            ...
            if search_thread is not None and not search_thread.is_alive():
                file_display = search_result.get('file_display')
                search_thread = None # Reset the thread

                if file_display and file_display.discogs_results:
                    # We have a valid result, move to the display state
                    app_state = "file_display"
                else:
                    # No results found or an error occurred, skip to the next file
                    if file_display: # Check if file_display object was created
                        print(f"No Discogs results for {file_display.filename}, skipping.")
                    current_file_index += 1
                    file_display = None
