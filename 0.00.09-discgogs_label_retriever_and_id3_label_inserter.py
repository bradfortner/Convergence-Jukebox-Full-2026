"""
Music File Cleaner v0.00.06
Add metadata display and Use This Label functionality to FinalRecordViewer
        ...
        try:
            ...
            if not cropped_label_surface:
                ...
                if len(self.label_images_surfaces) > 1:
                    ...
                    print("⚠ NO MORE LABEL IMAGES TO TRY - All images failed validation")
                    print("   Try pressing 'Next Discogs File' button to load a different release")
                    self.adaptor_preview_surface = None
                return
            ...
            text_valid = validate_label_text('new_cutout_label.png', self.discogs_artist, self.discogs_title, debug=True)
...
def detect_circular_label(image_bytes):
    ...
    Returns True only if a valid centered 45rpm label is detected.
...
def validate_label_text(image_path, artist_name, song_title, debug=False):
    ...
    try:
        ...
        # Pass validation if either artist OR title has good match
        threshold = 0.60  # 60% similarity threshold
        passed = (artist_match_ratio >= threshold) or (title_match_ratio >= threshold)
        ...
        if debug:
            ...
            print(f"  Artist match score: {artist_match_ratio:.2f} (threshold: {threshold})")
            print(f"  Title match score:  {title_match_ratio:.2f} (threshold: {threshold})")
            print(f"")
            if passed:
                if artist_match_ratio >= threshold:
                    print(f"✓ OCR text validation: PASSED (artist matched)")
                else:
                    print(f"✓ OCR text validation: PASSED (title matched)")
            else:
                print(f"✗ OCR text validation: FAILED (neither artist nor title found in label text)")
            print(f"")
        ...
        if self.use_label_checkbox.checked and not was_checked_before:
            if not self.image_surface:
                ...
                text_valid = validate_label_text('new_cutout_label.png',
                                                  self.data_bundle['discogs_artist'],
                                                  self.data_bundle['discogs_title'],
                                                  debug=True)
