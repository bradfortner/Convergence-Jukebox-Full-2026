...
class RenamerApp:
    ...
    def edit_item(self, event):
        ...
        if not item_id or column_id != "#3": # Column #3 is "Suggested"
            return

        x, y, width, height = self.file_list.bbox(item_id, column_id)
        current_text = self.file_list.item(item_id, "values")[2]
        ...
        entry.bind("<Return>", lambda e: self.save_edit(e, item_id, entry))
        entry.bind("<FocusOut>", lambda e: self.save_edit(e, item_id, entry))

    def save_edit(self, event, item_id, entry):
        new_text = entry.get()
        # Sanitize text after manual edit as well
        ...
        self.file_list.set(item_id, "Suggested", sanitized_text)
        entry.destroy()
