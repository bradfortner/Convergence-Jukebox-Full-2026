...
Version 0.91.01 - Change Access Code Functionality

This version implements a full three-step workflow to change the operator access code,
with persistent storage in user_config.txt.
...
def load_access_code():
    """
    Load operator access code from user_config.txt.
        ...
        if os.path.exists('user_config.txt'):
            with open('user_config.txt', 'r') as f:
                ...
                if len(code) == 4 and all(d in '1234567' for d in code):
                    print(f"[INIT] Loaded access code from user_config.txt")
                    return code
                else:
                    print(f"[WARNING] Invalid access code in user_config.txt, using default")
                    return ['7', '7', '7', '7']
        else:
            print(f"[INIT] user_config.txt not found, using default access code")
            # Create file with default code
...
def save_access_code(code):
    """
    Save operator access code to user_config.txt.
