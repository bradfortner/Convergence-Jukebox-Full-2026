"""
Music File Cleaner v0.00.13
GENRE SEARCH - Search music directory by genre (from ID3 Comment tag)
...
class GenreSelectionScreen:
    ...
    def __init__(self, screen, music_files):
        ...
        # Scan all music files to collect unique genres
        print("Scanning all music files for genres...")
        self.genres = scan_all_genres(music_files)
        ...
        for i, genre in enumerate(self.genres):
            ...
            checkbox = Checkbox(checkbox_x, y_pos, 20, 20, genre)
...
def scan_all_genres(music_files):
    """
    ...
    print(f"Scanning {len(music_files)} files to collect unique genres...")
    ...
    for music_file in music_files:
        try:
            ...
            # Read ID3 tags using mutagen
            audio = mutagen.File(file_path, easy=True)
            if audio is None:
                continue

            # Get Comment tag (COMM)
            comment = audio.get('comment', [''])[0] if 'comment' in audio else ''

            # Extract genre from comment (strip "image" and "noimage" suffixes)
            if comment:
                # Remove " image" or " noimage" suffix
                genre = comment.strip()
                if genre.endswith(' image'):
                    genre = genre[:-6].strip()
                elif genre.endswith('image'):
                    genre = genre[:-5].strip()
                elif genre.endswith(' noimage'):
                    genre = genre[:-8].strip()
                elif genre.endswith('noimage'):
                    genre = genre[:-7].strip()

                # Add non-empty genres to set
...
def find_files_by_genre(music_files, genre_search):
    ...
    for music_file in music_files:
        try:
            ...
            if genre_lower in comment.lower():
...
def main():
    ...
    pygame.display.set_caption("Music File Cleaner v0.00.13")
                    ...
                    if search_config and search_config['mode'] == 'from_discogs_list':
                        ...
                        if search_config['list_index'] >= len(search_config['genre_files']):
                            ...
                            if current_file_index == -1:
                                ...
                                current_file = music_files[current_file_index]
                                search_result.clear()
                                bypass = True  # Bypass skip logic for genre search
                                search_thread = threading.Thread(target=search_worker, args=(current_file, bypass))
                                search_thread.start()
                    else:
                        current_file = music_files[current_file_index]
