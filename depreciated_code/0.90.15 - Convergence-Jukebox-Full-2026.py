...
Version 0.90.15 - Pure Pygame Implementation (Fix: C Button Keypress Assignment)
...
Version 0.90.15 Changes:
- Fixed C button keypress: lowercase 'c' now selects column C (only when no selection)
- Fixed CORRECT button keypress: uppercase 'C' (Shift+C) now clears selection
- Matches original behavior from 0.83.60 version
- Previous bug: lowercase 'c' tried to both select AND clear depending on state
- Previous bug: uppercase 'C' was completely non-functional
...
def main():
    ...
    print("Convergence Jukebox v0.90.15 - Fix C Button Keypress Assignment")
