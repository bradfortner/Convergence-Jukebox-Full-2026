...
class ButtonGrid:
    ...
    def __init__(self, start_x, start_y, song_list, selection_window_number=0):
        ...
        # Create fonts
        self.font_id = pygame.font.SysFont('liberationsans', 16, bold=True)
        self.font_song = pygame.font.SysFont('liberationsans', 14, bold=True)
...
class InfoScreen:
    ...
    def __init__(self, start_x, start_y, song_list):
        ...
        # Create fonts
        self.font_header_large = pygame.font.SysFont('liberationsans', 26, bold=True)
        self.font_header_medium = pygame.font.SysFont('liberationsans', 18, bold=True)
        self.font_song_title = pygame.font.SysFont('liberationsans', 18, bold=True)
        self.font_song_artist = pygame.font.SysFont('liberationsans', 16, bold=True)
        self.font_info = pygame.font.SysFont('liberationsans', 12, bold=True)
        self.font_credits = pygame.font.SysFont('liberationsans', 26, bold=True)
...
class PlaybackEngine:
    ...
    def __init__(self, song_list, paid_playlist_path):
        ...
        # Initialize VLC with audio device debugging enabled
        # Use ALSA for better Bluetooth audio support on Raspberry Pi
        print("[AUDIO DEBUG] Initializing VLC audio engine...")
        vlc_args = ['--quiet', '--no-video', '--aout=alsa']
        self.vlc_instance = vlc.Instance(*vlc_args)
        self.player = self.vlc_instance.media_player_new()
        print("[AUDIO DEBUG] VLC initialized with ALSA audio backend")
        self.current_song_index = None
        self.paid_playlist = []
    ...
    def play_next_song(self):
        ...
        if len(self.paid_playlist) > 0:
            ...
            if song_index < len(self.song_list):
                ...
                print(f"[AUDIO DEBUG] Song file exists: {os.path.exists(song_path)}")
                ...
                self.player.play()
                print(f"[AUDIO DEBUG] Player state: {self.player.get_state()}")
...
def main():
    ...
    if os.path.exists(SONG_LIST_PATH):
        ...
        if current_file_count == stored_file_count and current_file_count != -1:
            ...
            try:
                ...
                song_list.sort(key=lambda x: x.get('artist', '').lower())
                print(f"[LOADED] {len(song_list)} songs from {SONG_LIST_PATH} (sorted by artist)")
    ...
    if not song_list:
        ...
        print("="*60)
        print(f"SONG LIST GENERATION COMPLETE: {len(song_list)} songs (sorted by artist)")
        print("="*60 + "\n")
    ...
    # Set SDL audio driver to ALSA for better Bluetooth support on Raspberry Pi
    print("[AUDIO DEBUG] Setting SDL audio driver for Bluetooth support...")
    os.environ['SDL_AUDIODRIVER'] = 'alsa'

    # Initialize Pygame
    print("[AUDIO DEBUG] Initializing pygame...")
    pygame.init()
    print("[AUDIO DEBUG] Initializing pygame mixer with ALSA backend...")
    try:
        pygame.mixer.init()
        print(f"[AUDIO DEBUG] Pygame mixer initialized successfully")
        print(f"[AUDIO DEBUG] Mixer channels: {pygame.mixer.get_num_channels()}")
    except Exception as mixer_error:
        print(f"[ERROR] Pygame mixer initialization failed: {mixer_error}")
        print("[WARNING] Sound effects will not be available")
    ...
    print("Press T for title search, A (caps lock or shift) for artist search")
