        ...
        if (event) == "--TITLE_SEARCH--" or (event) == "T" or (event) == "--ARTIST_SEARCH--" or (event) == "A":
            ...
            while True:
                ...
                if event == "-NEXT-" or event == "-PREV-" or event == "--CLEAR--" or event == '--EXIT--':
                    ...
                    print(f"[VISIBILITY DEBUG] Setting visibility for {len(search_results)} results")
                    if len(search_results) >= 5:
                        print(f"[VISIBILITY DEBUG] len >= 5, showing 5 buttons")
                    ...
                    for x in range(len(search_results)):
                        ...
                        if len(search_results) > 5:
                            print(f"[TEXT DEBUG] Updating buttons with first 5 of {len(search_results)} results")
                            print(f"[TEXT DEBUG] Button 1: {search_results[0][1]}")
                            search_window["--result_one--"].update(search_results[0][1], disabled=False)
                            search_window["--result_two--"].update(search_results[1][1])
                            search_window["--result_three--"].update(search_results[2][1])
                            search_window["--result_four--"].update(search_results[3][1])
