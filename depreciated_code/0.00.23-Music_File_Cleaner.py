...
class ConfirmationScreen:
    ...
    def _sanitize_text(self, text):
        ...
        sanitized = re.sub(r'[^A-Za-z ]', '', str(text))
        # Remove extra spaces
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key in (pygame.K_RETURN, pygame.K_ESCAPE):
                if focused_widget == self.back_button or event.key == pygame.K_ESCAPE:
                    ...
                    # TODO: For now, do nothing (placeholder for future functionality)
                    print("Confirm button pressed - functionality to be implemented")
                    return None
    ...
    def draw(self):
        ...
        for label_text, data_text in metadata:
            label_surf = self.font_label.render(label_text, True, (200, 200, 200))
            self.screen.blit(label_surf, (x_pos, y_offset))

            # Truncate long text
            if len(str(data_text)) > 60:
                data_text = str(data_text)[:57] + '...'

            data_surf = self.font_data.render(str(data_text), True, (255, 255, 255))
            self.screen.blit(data_surf, (x_pos + 100, y_offset))
