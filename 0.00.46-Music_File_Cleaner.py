...
class PreviewScreen:
    def __init__(self, screen, preview_data):
        self.screen = screen
        self.filename = preview_data['filename']
        self.final_title = preview_data['final_title']
        self.final_artist = preview_data['final_artist']
        self.final_year = preview_data['final_year']
        self.final_genre = preview_data['final_genre']
        self.final_comment = preview_data['final_comment']
        self.show_image = preview_data['show_image']

        self.font_header = pygame.font.Font(None, 32)
        self.font_info = pygame.font.Font(None, 22)

        # Load the preview image FRESH from disk (bypass pygame cache)
        self.preview_image = None
        if self.show_image:
            if os.path.exists('final_record.png'):
                file_size = os.path.getsize('final_record.png')
                print(f"PreviewScreen: Reading final_record.png from disk ({file_size} bytes)")
                if file_size > 0:
                    try:
                        # Use PIL to force fresh read from disk, then convert to pygame
                        from PIL import Image
                        pil_image = Image.open('final_record.png')
                        mode = pil_image.mode
                        size = pil_image.size
                        data = pil_image.tobytes()

                        # Convert PIL image to pygame surface
                        self.preview_image = pygame.image.fromstring(data, size, mode)
                        print(f"PreviewScreen: Successfully loaded FRESH image from disk ({size[0]}x{size[1]})")
                    except Exception as e:
                        print(f"PreviewScreen: Error loading preview image: {e}")
                else:
                    print("PreviewScreen: WARNING - final_record.png is empty!")
            else:
                print("PreviewScreen: WARNING - final_record.png does not exist!")

        # Buttons
        button_y = 600
        self.back_button = Button(screen.get_width() // 2 - 250, button_y, 200, 32, "Back")
        self.confirm_write_button = Button(screen.get_width() // 2 + 50, button_y, 200, 32, "Confirm & Write")

        self.focusable_widgets = [self.back_button, self.confirm_write_button]
        self.focused_index = 0
        self.focusable_widgets[self.focused_index].focused = True
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_TAB:
                ...
                focused_widget = self.focusable_widgets[self.focused_index]
                if focused_widget == self.back_button:
                    return "back_to_confirmation", None
                elif focused_widget == self.confirm_write_button:
                    return self._write_tags()

        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.back_button.rect.collidepoint(event.pos):
                return "back_to_confirmation", None
            if self.confirm_write_button.rect.collidepoint(event.pos):
                return self._write_tags()

        return None, None

    def _write_tags(self):
        """Write ID3 tags and transition to UpdatedTagsViewer"""
        print("\n=== WRITING ID3 TAGS ===")
        print(f"Title: {self.final_title}")
        print(f"Artist: {self.final_artist}")
        print(f"Year: {self.final_year}")
        print(f"Genre: {self.final_genre}")
        print(f"Comment: {self.final_comment}")
        print("========================\n")

        # Determine image path
        image_path = 'final_record.png' if self.show_image else None
    ...
    def draw(self):
        # Header
        header = self.font_header.render("Preview Final Record", True, (255, 255, 0))
        self.screen.blit(header, (50, 50))

        # Info text
        info = self.font_info.render(f"File: {self.filename}", True, (200, 200, 200))
        self.screen.blit(info, (50, 90))

        # Display preview image if available
        if self.preview_image:
            # Scale to fit screen (max 400x400)
            max_size = 400
            image_rect = self.preview_image.get_rect()
            ...
            scaled_image = pygame.transform.scale(self.preview_image, (new_width, new_height))

            # Center the image
            image_x = (self.screen.get_width() - new_width) // 2
            image_y = 150
            self.screen.blit(scaled_image, (image_x, image_y))

            # Label under image
            label_text = self.font_info.render("Combined: blank_record + adaptor + label", True, (150, 150, 150))
            label_rect = label_text.get_rect(center=(self.screen.get_width() // 2, image_y + new_height + 20))
            self.screen.blit(label_text, label_rect)
        else:
            # No image mode or error loading
            no_img_text = self.font_info.render("Data-only mode (no image)", True, (150, 150, 150))
            no_img_rect = no_img_text.get_rect(center=(self.screen.get_width() // 2, 300))
            self.screen.blit(no_img_text, no_img_rect)

        # Buttons
        self.back_button.draw(self.screen)
        self.confirm_write_button.draw(self.screen)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.show_confirmation_buttons:
                if self.are_you_sure_button.rect.collidepoint(event.pos):
                    ...
                    if self.show_image:
                        # Delete old final_record.png to ensure fresh creation
                        if os.path.exists('final_record.png'):
                            os.remove('final_record.png')
                            print("Deleted old final_record.png")

                        print("Creating NEW final record label...")
                        ...
                        if success:
                            print(f"NEW final_record.png created ({os.path.getsize('final_record.png')} bytes)")
                        else:
                            print("WARNING: Failed to create final record label.")

                    # Go to preview screen (NO FILE WRITE YET)
                    preview_data = {
                        'filename': self.filename,
...
def main():
    ...
    while not done:
        ...
        for event in pygame.event.get():
            ...
            if app_state == "file_display" and file_display:
                ...
                if result:
                    ...
                    if action == "back_to_result":
                        ...
                        preview_screen = PreviewScreen(screen, data)
                        app_state = "preview"
                        confirmation_screen = None
            ...
            elif app_state == "preview" and preview_screen:
                result = preview_screen.handle_event(event)
                if result:
                    action, data = result if isinstance(result, tuple) else (result, None)
