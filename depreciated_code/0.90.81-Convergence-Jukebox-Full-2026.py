...
def generate_record_image(song_title, artist_name, year=None, song_file_path=None):
...
def generate_selection_record_label(song_title, artist_name, available_labels, year=None, song_file_path=None):
    """
...
class PlaybackEngine:
    ...
    def load_genre_flags(self):
        """Load genre filter flags from user_settings.txt via load_settings() helper.

        Updates the 4 genre filter attributes based on the 'genres' list in settings.
        """
        try:
            settings = load_settings()
            genre_list = settings.get('genres', ['null', 'null', 'null', 'null'])
            
            # Ensure we have at least 4 elements, pad with 'null' if needed
            while len(genre_list) < 4:
                genre_list.append('null')
                
            self.genre0 = genre_list[0]
            self.genre1 = genre_list[1]
            self.genre2 = genre_list[2]
            self.genre3 = genre_list[3]
...
USER_SETTINGS_PATH = "user_settings.txt"


def load_settings():
    """Loads settings from user_settings.txt, creating it with defaults if it doesn't exist."""
    defaults = {
        'access_code': '7777',
    ...
    if not os.path.exists(USER_SETTINGS_PATH):
        print(f"[SETTINGS] '{USER_SETTINGS_PATH}' not found, creating with default values.")
        try:
            with open(USER_SETTINGS_PATH, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['access_code', defaults['access_code']])
                writer.writerow(['genres'] + defaults['genres'])
            print(f"[SETTINGS] Created '{USER_SETTINGS_PATH}' with default settings.")
        except Exception as e:
            print(f"[ERROR] Failed to create settings file '{USER_SETTINGS_PATH}': {e}")
        return defaults

    settings = {}
    try:
        with open(USER_SETTINGS_PATH, 'r', newline='') as f:
            reader = csv.reader(f)
            for row in reader:
                if not row:
                    continue
                key = row[0]
                if key == 'genres':
                    settings[key] = row[1:] if len(row) > 1 else []
                else:
                    settings[key] = row[1] if len(row) > 1 else ''
        
        # Ensure all default keys are present and handle empty file case
        if 'access_code' not in settings or not settings['access_code']:
            settings['access_code'] = defaults['access_code']
        if 'genres' not in settings or not settings['genres']:
            settings['genres'] = defaults['genres']
            
        print(f"[SETTINGS] Loaded settings from '{USER_SETTINGS_PATH}'.")
        return settings
    except Exception as e:
        print(f"[ERROR] Failed to load settings from '{USER_SETTINGS_PATH}': {e}. Using defaults.")
        return defaults

def save_setting(key_to_save, value_to_save):
    """Saves a specific setting to user_settings.txt, preserving other values."""
    # First, load all current settings from the file to a dictionary.
    settings = {}
                ...
                for row in reader:
                    if row:
                        if row[0] == 'genres':
                             ...
                             settings[row[0]] = row[1:] # Store as list to handle single values consistently in writer
        except Exception as e:
            print(f"[ERROR] Could not read existing settings for update: {e}")

    # Update the specific key with the new value. Ensure value is a list for CSV writer.
    settings[key_to_save] = [value_to_save] if not isinstance(value_to_save, list) else value_to_save
    
    # Write all settings back to the file
    try:
        with open(USER_SETTINGS_PATH, 'w', newline='') as f:
            writer = csv.writer(f)
            for key, value in settings.items():
                writer.writerow([key] + value)
        print(f"[SETTINGS] Saved '{key_to_save}' to '{USER_SETTINGS_PATH}'.")
    except Exception as e:
        print(f"[ERROR] Failed to save settings to '{USER_SETTINGS_PATH}': {e}.")
                            ...
                            if song_index < len(song_list):
                                ...
                                try:
                                    ...
                                    artist_name = apply_the_prefix(
                                        artist_name, the_bands_set
                                    )
                                    # Build song file path for ID3 extraction
                                    song_file_path = os.path.join(MUSIC_DIRECTORY, f"{song['artist']} - {song['title']}.mp3")
            ...
            if playback_engine.player and playback_engine.current_song_index is not None:
                ...
                if current_time_ms > 0 and duration_ms > 0:
                    ...
                    if should_show:
                        ...
                        # Generate record image (with ID3 priority)
