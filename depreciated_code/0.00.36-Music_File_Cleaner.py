...
import mp3gain_normalization_module
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.show_confirmation_buttons:
                if self.are_you_sure_button.rect.collidepoint(event.pos):
                    # TODO: Implement file write logic later
                    ...
                    # Only create and display image if in image mode
                    if self.show_image:
                        # Combine label images and display result
                        ...
                        if success:
                            print("Displaying final record label...")
                            label_pressing_module.display_image('final_record.png')
                        else:
                            print("Failed to create final record label.")

                        # Normalize audio with mp3gain
                        print(f"\nNormalizing audio for {self.filename}...")
                        mp3gain_normalization_module.normalize_mp3(self.filename)
                    else:
                        # Data-only mode - normalize audio
                        print(f"\nNormalizing audio for {self.filename}...")
                        mp3gain_normalization_module.normalize_mp3(self.filename)
