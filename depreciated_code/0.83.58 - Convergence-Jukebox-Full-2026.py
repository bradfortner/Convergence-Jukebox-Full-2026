...
Version 0.83.58 - Temporarily Disable Topmost on FreeSimpleGUI Windows
...
Version 0.83.58 Changes:
- FINAL FIX: Temporarily disable HWND_TOPMOST on FreeSimpleGUI windows during popup
- Root cause: Pygame window created successfully but stuck BEHIND topmost FreeSimpleGUI
- Solution: Use SetWindowPos with HWND_NOTOPMOST to remove topmost flag temporarily
- When popup shows: Disable topmost on background/info/arrow windows
- When popup closes: Re-enable topmost on background/info/arrow windows  
- This allows pygame window to appear without hiding jukebox interface
- Pygame window can now be seen because it's not competing with topmost windows

Version 0.83.57 Changes:
- NEW APPROACH: Force pygame window topmost using 5-step Windows API sequence
- Step 1: SetWindowLong with WS_EX_TOPMOST | WS_EX_LAYERED flags
- Step 2: MoveWindow to position (553, 200)
- Step 3: ShowWindow with SW_SHOW
- Step 4: SetForegroundWindow + BringWindowToTop to force focus
- Step 5: SetWindowPos with HWND_TOPMOST and SWP_NOMOVE | SWP_NOSIZE
- Keep background/info/arrow windows visible (don't hide them)
- Only hide selection windows to prevent interaction conflicts
- Popup overlays on top of jukebox interface instead of replacing it

Version 0.83.56 Changes:
- CRITICAL FIX: Hide ALL FreeSimpleGUI windows when pygame popup shows
- Root cause: Pygame window cannot become topmost (error 1400), so it was
  appearing BEHIND the topmost FreeSimpleGUI windows (especially window_background)
- Solution: Hide all 7 FreeSimpleGUI windows during popup, show only pygame
- Restore all windows when popup closes (keypress, song ending, or flag set)
- Popup is now visible because no topmost windows are in front of it

Version 0.83.55 Changes:
- Fixed ERROR_INVALID_WINDOW_HANDLE (1400) in SetWindowPos
- Reverted to single SetWindowPos call (handle was invalidating between calls)
- Added IsWindow() validation before API calls
- Added fallback method using SetWindowLong + MoveWindow if SetWindowPos fails
- Window handle is now validated before use

Version 0.83.54 Changes:
- Fixed SetWindowPos failure causing pygame window to be invisible
- Split positioning and topmost setting into two separate API calls
- Added GetLastError() reporting for diagnostic information
- Increased window initialization delay from 0.1s to 0.3s
- Popup window now properly appears on screen and stays on top

Version 0.83.53 Changes:
- Added debug logging for rotating record popup conditions
- Enhanced exception handling to log errors instead of silently passing
- Logs popup condition values every 5 seconds to help diagnose issues

Version 0.83.52 Changes:
- All FreeSimpleGUI windows now use keep_on_top=True
- Pygame rotating record popup set to HWND_TOPMOST
- Prevents external applications from appearing above jukebox interface
"""
...
from search_module import run_search
...
# Create a global VLC instance with arguments to suppress error messages
vlc_instance = vlc.Instance('--quiet', '--no-video')
...
class JukeboxEngine:
    ...
    def play_song(self, song_file_name: str) -> bool:
        ...
        try:
            ...
            try:
                p: vlc.MediaPlayer = vlc_instance.media_player_new(song_file_name)
...
def create_vlc_player_silent(file_path):
    ...
    try:
        ...
        with open(os.devnull, 'w') as devnull:
            ...
            player = vlc_instance.media_player_new(file_path)
    finally:
...
def main():
    ...
    while True:
            ...
            if event.startswith('--') and ('KEY' in event or 'PRESSED' in event or event == '--ESC--'):
                ...
                if rotating_record_rotation_stop_flag is not None:
                    try:
                        ...
                        # Re-enable topmost on visible windows
                        import ctypes
                        HWND_TOPMOST = -1
                        SWP_NOMOVE = 0x0002
                        SWP_NOSIZE = 0x0001
                        ...
                        for window in [window_background, info_screen_window, right_arrow_selection_window, left_arrow_selection_window]:
                            try:
                        ...
                        # Restore selection windows (background was never hidden)
        ...
        if (event) == "T" or (event) == "A":
            ...
            # Prepare window references for search module
            main_windows = {
                'right_arrow_selection_window': right_arrow_selection_window,
                'left_arrow_selection_window': left_arrow_selection_window,
                'jukebox_selection_window': jukebox_selection_window,
                'info_screen_window': info_screen_window,
                'control_button_window': control_button_window,
                'song_playing_lookup_window': song_playing_lookup_window,
                'window_background': window_background
            }

            # Prepare callback functions for search module
            callback_functions = {
                'selection_buttons_update': selection_buttons_update,
                'disable_a_selection_buttons': disable_a_selection_buttons,
                'disable_b_selection_buttons': disable_b_selection_buttons,
                'disable_c_selection_buttons': disable_c_selection_buttons
            }
            ...
            if search_result is not None:
                # Song was selected
                song_selected_number = search_result['song_number']
                if search_result['song_selected'] is not None:
                    song_selected = search_result['song_selected']
                    ...
                    if MusicMasterSongList[counter]['location'] == song_currently_playing:
                        ...
                        try:
                            ...
                            if current_time_ms > 0 and duration_ms > 0:
                                ...
                                # Debug logging every 5 seconds to track popup conditions
                                current_time_str = datetime.now().strftime("%H:%M:%S")
                                if int(elapsed_seconds) % 5 == 0 and int(elapsed_seconds) > 0:
                                    debug_msg = f"POPUP DEBUG: elapsed={elapsed_seconds:.1f}s, idle={time_since_keypress:.1f}s, remaining={time_remaining_seconds:.1f}s, flag={'None' if rotating_record_rotation_stop_flag is None else 'Set'}"
                                    print(debug_msg)
                                    with open('log.txt', 'a') as log:
                                        log.write(f'\n{current_time_str}, {debug_msg}')
                                ...
                                if should_show:
                                    print(f"POPUP: Showing rotating record popup - all conditions met!")
                                    with open('log.txt', 'a') as log:
                                        log.write(f'\n{current_time_str}, POPUP: Showing rotating record popup')
                                    
                                    # CRITICAL: Temporarily disable topmost on visible windows so pygame can appear on top
                                    print("Disabling topmost flags on FreeSimpleGUI windows...")
                                    import ctypes
                                    HWND_NOTOPMOST = -2
                                    SWP_NOMOVE = 0x0002
                                    SWP_NOSIZE = 0x0001
                                    ...
                                    # Get window handles and disable topmost
                                    for window in [window_background, info_screen_window, right_arrow_selection_window, left_arrow_selection_window]:
                                        try:
                                            hwnd = window.TKroot.winfo_id()
                                            ...
                                            pass
                                    print("✓ Topmost disabled on visible windows")
                                    
                                    # Hide selection windows (prevent interaction)
                                ...
                                if should_close:
                                    ...
                                    # Re-enable topmost on visible windows
                                    print("Re-enabling topmost flags on FreeSimpleGUI windows...")
                                    import ctypes
                                    HWND_TOPMOST = -1
                                    SWP_NOMOVE = 0x0002
                                    SWP_NOSIZE = 0x0001
                                    ...
                                    for window in [window_background, info_screen_window, right_arrow_selection_window, left_arrow_selection_window]:
                                        try:
                                            ...
                                            pass
                                    print("✓ Topmost re-enabled on visible windows")
                            ...
                            current_time_str = datetime.now().strftime("%H:%M:%S")
                            error_msg = f"POPUP ERROR: {str(e)}"
                            print(error_msg)
                            with open('log.txt', 'a') as log:
                                log.write(f'\n{current_time_str}, {error_msg}')
                            import traceback
                            traceback.print_exc()
