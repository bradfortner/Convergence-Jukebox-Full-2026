"""
Music File Cleaner v0.00.39
CRITICAL FIX: Replace Unicode characters causing Windows console crash

CHANGELOG v0.00.39:
- **CRITICAL UNICODE FIX**:
  1. Program was CRASHING on Windows due to Unicode checkmarks in print statements
  2. Replaced all Unicode characters with ASCII equivalents:
     - [OK] instead of checkmark (Unicode U+2713)
     - [X] instead of X-mark (Unicode U+2717)
     - [!] instead of warning (Unicode U+26A0)
     - >> instead of skip arrow (Unicode U+23ED)
  3. Program will now actually RUN instead of crashing on startup!

CHANGELOG v0.00.38:
- **DEBUG VERIFICATION FLAGS**:
  1. Shows verification_passed, structural_valid, ocr_valid after FileDisplay is created
  2. This will reveal if flags are being set during __init__ or if timing is the issue

CHANGELOG v0.00.37:
- **TRUE AUTO-JUMP IMPLEMENTATION**:
  1. After FileDisplay is created, immediately checks if verification_passed is True
  2. If verified: IMMEDIATELY creates ConfirmationScreen (NO WAITING, NO CLICK)
  3. If not verified: Shows FileDisplay for manual review
  4. User sees ZERO screens when label is verified - goes straight to "Are You Sure"
  5. THIS IS WHAT WAS ACTUALLY REQUESTED - automatic jump on verification, not on click

CHANGELOG v0.00.36:
- **MORE DEBUG LOGGING**:
  1. Added debug output in show_confirmation handler to verify it's being called
  2. Shows data structure being passed to ConfirmationScreen
  3. Tracks app_state transition from file_display to confirmation

CHANGELOG v0.00.35:
- **CRITICAL FIX - AUTO-JUMP NOW WORKS**:
  1. Added missing "show_confirmation" action handler in FileDisplay event loop
  2. Previously, action was returned but never handled - no transition happened
  3. Now properly jumps from FileDisplay → ConfirmationScreen when label is verified
  4. THIS ACTUALLY FIXES THE AUTO-JUMP FEATURE

CHANGELOG v0.00.34:
- **DEBUG LOGGING**:
  1. Added debug output when adaptor preview is clicked
  2. Shows verification_passed, structural_valid, and ocr_valid status
  3. Helps diagnose why auto-jump may not be working

CHANGELOG v0.00.33:
- **AUTO-JUMP TO CONFIRMATION WHEN VERIFIED**:
  1. Verification now happens automatically when adaptor preview is generated on FileDisplay
  2. When both structural and OCR validation pass, verification status is stored
  3. Clicking adaptor preview when VERIFIED: Jumps directly to ConfirmationScreen (skips FinalRecordViewer)
  4. Clicking adaptor preview when NOT VERIFIED: Goes to FinalRecordViewer for manual review
  5. Fully automated workflow for verified labels - one click from preview to confirmation!

- **STREAMLINED WORKFLOW**:
  - Before: FileDisplay → Click preview → FinalRecordViewer → Check box → ConfirmationScreen
  - After (VERIFIED): FileDisplay → Click preview → ConfirmationScreen (automatic!)
  - After (NOT VERIFIED): FileDisplay → Click preview → FinalRecordViewer (manual review)

CHANGELOG v0.00.32:
- **AUTO-SELECT VERIFIED LABELS**:
  1. When label passes verification (green "VERIFIED [OK]"), automatically checks "Use This Label" checkbox
  2. Saves user one click - can immediately proceed to ConfirmationScreen
  3. Only auto-checks when BOTH structural and OCR validation pass

- **USE OCR-VERIFIED ARTIST/TITLE**:
  1. When verification passes, stores OCR-verified artist/title from filename
  2. ConfirmationScreen now uses OCR-verified data instead of Discogs data
  3. Title and Artist fields show the actual text that was found on the label
  4. More accurate - uses what was actually read from the physical label
  5. Discogs data only used as fallback when verification doesn't pass

CHANGELOG v0.00.31:
- **AUTOMATED VERIFICATION SYSTEM**:
  1. When FinalRecordViewer opens, automatically runs two validation checks:
     - Structural validation: Checks if final_record.png has proper 45rpm label structure
     - OCR validation: Checks if artist/title from filename match label text (70% threshold)
  2. Displays prominent visual banner at top of screen:
     - Green "VERIFIED [OK]" when both checks pass
     - Yellow/Orange warning when checks fail with specific reason
  3. Shows detailed status: "Structural: [OK]/[X]  |  OCR Match: [OK]/[X]"
  4. Prints comprehensive validation report to console
  5. Helps user quickly assess label quality without manual review

CHANGELOG v0.00.30:
- **IMPROVED CIRCLE DETECTION PRE-FILTER**:
  1. Relaxed detect_circular_label() to match extract_label() flexibility
  2. Added 4 detection strategies (was 2) with varying sensitivity levels
  3. Increased center offset tolerance from 25% to 35%
  4. Widened radius range from 0.20-0.50 to 0.15-0.55
  5. Pre-filter now INCLUSIVE - OCR validation (70% threshold) does the real filtering
  6. Should capture ~30% more valid labels that were previously rejected

CHANGELOG v0.00.29:
- **FIXED ENHANCEDINPUTBOX TEXT EDITING**:
  1. Fixed cursor positioning to use actual font metrics instead of crude character width estimation
  2. Now accurately positions cursor when clicking anywhere in text (character-by-character measurement)
  3. Added Ctrl+C copy and Ctrl+X cut functionality
...
- **INCREASED AUTO-ADVANCE LIMIT**:
  1. Raised max_auto_advance_attempts from 5 to 20 releases
  2. Allows searching through more Discogs results before stopping
  3. Reduces need for manual "Next Discogs File" clicks

CHANGELOG v0.00.27:
- **AUTO-ADVANCE FIX**:
  1. Fixed issue where process stops after first Discogs result when all labels fail OCR
  2. Now auto-advances to next Discogs result when initial result has zero valid labels
  3. Triggers auto-advance in FileDisplay.__init__() after processing prefetched data
  4. Continues trying up to max_auto_advance_attempts (default: 5 releases)

CHANGELOG v0.00.26:
- **IMPROVED OCR TEXT MATCHING**:
  1. Switched from SequenceMatcher to rapidfuzz library for better fuzzy matching
  2. Raised match thresholds from 50% to 70% to reduce false positives
  3. Added adaptive thresholding preprocessing for crisp black/white text
  4. Changed Tesseract PSM from 6 to 11 for better scattered text detection
  5. Added OEM 1 (LSTM engine) for improved accuracy
  6. Added image upscaling for small images (< 600px)
  7. Uses token_set_ratio (handles word reordering) and partial_ratio (substring matching)

CHANGELOG v0.00.25:
- **ENHANCED TEXT INPUT (EnhancedInputBox)**:
  1. Arrow keys (left/right) to move cursor within text
  2. Home/End keys to jump to start/end of text
  3. Backspace deletes character before cursor
  4. Delete key removes character after cursor
  5. Ctrl+V paste from clipboard
  6. Ctrl+A select all text
  7. Click to position cursor at click location
  8. Blinking cursor visible at current position

- **BACKGROUND PREFETCH SYSTEM (PrefetchManager)**:
  1. Prefetches next 2 files while user reviews current file
  2. Uses dedicated thread pool for background processing
  3. Caches prefetched data (Discogs results, images, OCR)
  4. Zero wait time between files when prefetch completes
  5. Automatic prefetch queue management

- **ENHANCED STATUS BAR UI**:
  1. Persistent status bar at bottom of screen (30px height)
  2. Shows "File X of Y" counter for queue visibility
  3. Processing stages: Idle, Searching, Downloading, OCR, Ready
  4. Prefetch count indicator when files being prefetched

- **IMPROVED LRU CACHING**:
  1. Uses OrderedDict for LRU (Least Recently Used) eviction
  2. Increased MAX_CACHE_SIZE from 100 to 500 items
  3. Evicts oldest items first instead of clearing entire cache
  4. Cache hit/miss tracking for debugging

CHANGELOG v0.00.24:
- **API RETRY WITH EXPONENTIAL BACKOFF**:
  1. Added discogs_api_call_with_retry() wrapper function for robust API calls
  2. Retries failed API calls up to 3 times with exponential backoff (2s, 4s, 8s)
  3. Handles empty responses (JSON decode errors) gracefully
  4. Logs retry attempts for debugging
  5. Applied to all d.search() and d.release() calls
  6. Prevents crashes from transient API failures or server-side throttling

CHANGELOG v0.00.23:
- **DISCOGS API RATE LIMITING**:
  1. Added configurable delay between Discogs API requests
  2. DISCOGS_API_DELAY setting in music_file_cleaner_config.py (default: 1.5 seconds)
  3. Prevents 429 "Too Many Requests" errors
  4. Tracks last API call time to enforce minimum delay
  5. Applied to both d.search() and d.release() calls
...
from rapidfuzz import fuzz  # v0.00.26: Better fuzzy string matching
import hashlib
...
try:
    ...
    OCR_AVAILABLE = True
    print("[OK] OCR (pytesseract) successfully loaded - text validation enabled")
except ImportError:
    OCR_AVAILABLE = False
...
# v0.00.25: LRU Performance optimization caches (upgraded from dict to OrderedDict)
CIRCLE_DETECTION_CACHE = OrderedDict()  # Cache circle detection results by image hash (LRU)
OCR_CACHE = OrderedDict()  # Cache OCR results by image hash (LRU)
MAX_CACHE_SIZE = 500  # Increased from 100, uses LRU eviction now
CACHE_HITS = 0  # v0.00.25: Track cache hit count for debugging
CACHE_MISSES = 0  # v0.00.25: Track cache miss count for debugging


def cache_put(cache, key, value):
    """
    v0.00.25: LRU cache put operation.
    Moves existing keys to end (most recently used), evicts oldest when full.
    """
    if key in cache:
...
def cache_get(cache, key):
    """
    v0.00.25: LRU cache get operation.
    Returns value and moves to end (most recently used), or None if not found.
    """
...
class PrefetchManager:
    """
    v0.00.25: Background prefetching manager for upcoming files.
    Prefetches next N files while user reviews current file for zero wait time.
    """
    def __init__(self, max_prefetch=2, max_workers=2):
        self.cache = {}  # {filename: fetched_data}
        self.pending = set()  # Files currently being prefetched
        self.executor = ThreadPoolExecutor(max_workers=max_workers)
        self.lock = threading.Lock()
        self.max_prefetch = max_prefetch
        self.futures = {}  # {filename: future}

    def prefetch(self, music_files, current_index, search_config=None):
        """
        Start prefetching next files in queue.
        Called when user starts reviewing a file.
        """
        with self.lock:
            # Determine which files to prefetch
            files_to_prefetch = []
            for i in range(1, self.max_prefetch + 1):
                next_index = current_index + i
                if next_index < len(music_files):
                    filename = music_files[next_index]
                    # Skip if already cached or pending
                    if filename not in self.cache and filename not in self.pending:
                        files_to_prefetch.append((next_index, filename))

            # Submit prefetch tasks
            for idx, filename in files_to_prefetch:
                self.pending.add(filename)
                future = self.executor.submit(
                    self._prefetch_worker, filename, search_config
                )
                self.futures[filename] = future
                print(f"[Prefetch] Started prefetching: {filename}")

    def _prefetch_worker(self, music_file, search_config=None):
        """Worker function to prefetch a single file's data"""
        ...
        try:
            ...
            if 'image' in comment or 'noimage' in comment:
                print(f"[Prefetch] Skipping {music_file} - already processed")
                with self.lock:
                    self.pending.discard(music_file)
                return None
            ...
            # Search Discogs (uses rate limiting internally)
            results = search_discogs(f"{search_artist} - {title}")
            fetched_data['discogs_results'] = results

            if results:
                try:
                    release = discogs_api_call_with_retry(d.release, results[0].id)
                    if release:
                        fetched_data['full_release'] = release
                        # Download and filter images
                        filtered_images = download_and_filter_images_parallel(
                            release, artist_name=artist, song_title=title, max_workers=3
                        )
                        fetched_data['label_images_data'] = [
                            img_bytes for img_bytes, circle_data in filtered_images
                        ]
                except Exception as e:
                    print(f"[Prefetch] Error fetching release: {e}")

            # Store in cache
            with self.lock:
                self.cache[music_file] = fetched_data
                self.pending.discard(music_file)
                print(f"[Prefetch] Completed: {music_file}")

            return fetched_data

        except Exception as e:
            print(f"[Prefetch] Error for {music_file}: {e}")
            with self.lock:
                self.pending.discard(music_file)
            return None

    def get_cached(self, filename):
        """Return prefetched data if available, else None"""
        with self.lock:
            return self.cache.pop(filename, None)

    def is_pending(self, filename):
        """Check if file is currently being prefetched"""
        with self.lock:
            return filename in self.pending

    def wait_for(self, filename, timeout=30):
        ...
        if future:
            try:
                ...
                print(f"[Prefetch] Wait error for {filename}: {e}")
        return self.get_cached(filename)

    def clear_old(self, current_filename):
        """Remove cached data for files before current (already processed)"""
        with self.lock:
            # Find and remove old entries
            to_remove = []
            for cached_file in self.cache.keys():
                if cached_file != current_filename:
                    to_remove.append(cached_file)
            for f in to_remove:
                del self.cache[f]

    def get_prefetch_count(self):
        """Return number of files currently cached or being prefetched"""
        with self.lock:
            return len(self.cache) + len(self.pending)
...
PREFETCH_MANAGER = None  # Initialized in main()

# Initialize Discogs client
d = discogs_client.Client('YourApp/1.0', user_token=music_file_cleaner_config.DISCOGS_USER_TOKEN)

# v0.00.23: API Rate Limiting
LAST_API_CALL_TIME = 0  # Track last Discogs API call time

def apply_api_rate_limit():
    """
    Enforce minimum delay between Discogs API calls to prevent 429 errors.
    Sleeps if necessary to maintain DISCOGS_API_DELAY seconds between calls.
    """
    global LAST_API_CALL_TIME
    current_time = time.time()
    time_since_last_call = current_time - LAST_API_CALL_TIME

    if time_since_last_call < music_file_cleaner_config.DISCOGS_API_DELAY:
        sleep_time = music_file_cleaner_config.DISCOGS_API_DELAY - time_since_last_call
        print(f"Rate limiting: sleeping {sleep_time:.2f} seconds...")
        time.sleep(sleep_time)

    LAST_API_CALL_TIME = time.time()

# v0.00.24: Retry wrapper with exponential backoff
def discogs_api_call_with_retry(api_func, *args, max_retries=3, base_delay=2.0, **kwargs):
    """
    Wrapper to call Discogs API functions with retry logic and exponential backoff.

    Args:
        api_func: The Discogs API function to call (e.g., d.search, d.release)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                print(f"API returned empty response (attempt {attempt + 1}/{max_retries + 1}). Retrying in {delay:.1f}s...")
                logging.warning(f"Discogs API JSONDecodeError on attempt {attempt + 1}: {e}. Retrying in {delay}s...")
                time.sleep(delay)
            else:
                print(f"API failed after {max_retries + 1} attempts: {e}")
                logging.error(f"Discogs API call failed after {max_retries + 1} attempts: {e}")

        except Exception as e:
            last_exception = e
            if attempt < max_retries:
                delay = base_delay * (2 ** attempt)
                print(f"API error (attempt {attempt + 1}/{max_retries + 1}): {e}. Retrying in {delay:.1f}s...")
                logging.warning(f"Discogs API error on attempt {attempt + 1}: {e}. Retrying in {delay}s...")
                time.sleep(delay)
            else:
                print(f"API failed after {max_retries + 1} attempts: {e}")
                logging.error(f"Discogs API call failed after {max_retries + 1} attempts: {e}")
...
def search_discogs(query, page=1, search_type='release', max_results=10):
    ...
    release_list = []
    try:
        # v0.00.24: Use retry wrapper with exponential backoff
        start_time = time.time()
        results = discogs_api_call_with_retry(d.search, query, type=search_type)
        end_time = time.time()
...
class EnhancedInputBox:
    """
    v0.00.25: Enhanced text input with cursor positioning, selection, and clipboard support.
    Features: arrow keys, Home/End, Delete, Ctrl+V paste, Ctrl+A select all, click to position.
    """
    def __init__(self, x, y, w, h, text=''):
        self.rect = pygame.Rect(x, y, w, h)
        self.text = text
        self.placeholder_text = text
        self.cursor_pos = len(text)  # Cursor position in text
        self.selection_start = None  # For text selection (None = no selection)
        self.focused = False
        self.cursor_visible = True
        self.cursor_blink_timer = 0
        self.cursor_blink_interval = 500  # ms
        self.dragging = False  # v0.00.29: Track mouse drag for selection
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.rect.collidepoint(event.pos):
                ...
                self.cursor_pos = self._get_cursor_from_click(click_x)
                self.selection_start = None  # Clear selection on click
                self.dragging = True  # v0.00.29: Start drag selection
            else:
                self.focused = False

        # v0.00.29: Mouse drag selection
        if event.type == pygame.MOUSEMOTION and self.dragging and self.focused:
            if self.rect.collidepoint(event.pos):
                click_x = event.pos[0] - self.rect.x - 5
                new_pos = self._get_cursor_from_click(click_x)
                if new_pos != self.cursor_pos:
                    if self.selection_start is None:
                        self.selection_start = self.cursor_pos
                    self.cursor_pos = new_pos

        # v0.00.29: End drag selection
        if event.type == pygame.MOUSEBUTTONUP:
            self.dragging = False

        if event.type == pygame.KEYDOWN and self.focused:
            # Handle selection deletion first
            if self.selection_start is not None and event.key in (pygame.K_BACKSPACE, pygame.K_DELETE):
    ...
    def _get_cursor_from_click(self, click_x):
        """Calculate cursor position from click x-coordinate using actual font metrics"""
        if not self.text or click_x <= 0:
            return 0
        ...
        for i in range(len(self.text) + 1):
            ...
            if i < len(self.text):
                next_width = FONT.size(self.text[:i+1])[0]
                # Click is between this char and next - choose closest
                mid_point = (text_width + next_width) / 2
    ...
    def draw(self, screen):
        ...
        if self.focused and self.selection_start is not None and self.selection_start != self.cursor_pos:
            start = min(self.selection_start, self.cursor_pos)
            end = max(self.selection_start, self.cursor_pos)
            # Calculate selection rectangle
            pre_text = self.text[:start]
            sel_text = self.text[start:end]
            pre_width = FONT.size(pre_text)[0] if pre_text else 0
            sel_width = FONT.size(sel_text)[0] if sel_text else 0
            sel_rect = pygame.Rect(self.rect.x + 5 + pre_width, self.rect.y + 3,
                                   sel_width, self.rect.h - 6)
            pygame.draw.rect(screen, (70, 130, 180), sel_rect)  # Selection color
        ...
        if self.focused and self.cursor_visible:
            ...
            cursor_y2 = self.rect.y + self.rect.h - 4
...
class FileDisplay:
    def __init__(self, screen, filename, artist, title, fetched_data):
        ...
        # v0.00.33: Automated verification status (set when preview is generated)
        self.verification_passed = False
        self.structural_valid = False
        ...
        self.max_auto_advance_attempts = 20  # v0.00.28: Increased from 5 to 20 releases
        ...
        if self.full_release:
            ...
            # v0.00.27: Auto-advance if no valid label images found in initial result
            label_count = len(self.label_images_surfaces)
            if label_count == 0:
                print("[!] NO VALID LABEL IMAGES found in initial Discogs result (all failed OCR validation)")
                if self._try_next_discogs_release():
                    print("[RELOAD] Auto-advancing to next Discogs release...")
                else:
    ...
    def _generate_adaptor_preview(self, recursion_depth=0):
        ...
        try:
            ...
            print("[OK] final_record.png generated and validated successfully (420x420)")

            # v0.00.10: Reset auto-advance counter on success
            self.auto_advance_attempts = 0

            # v0.00.33: Set verification status (already validated above)
            # We already ran validate_final_record() and validate_label_text() above
            # If we got here, both passed (otherwise would have returned early)
            self.structural_valid = True  # Step 4 passed
            self.ocr_valid = True         # Step 5 passed
            self.verification_passed = True

            print("\n" + "="*60)
            print("AUTOMATED VERIFICATION (v0.00.33)")
            print("="*60)
            print("[OK] Structural validation: PASSED (valid 45rpm label structure)")
            print(f"[OK] OCR validation: PASSED ('{self.artist}' - '{self.title}' found on label)")
            print("\n" + "="*60)
            print("FINAL VERDICT: VERIFIED [OK]")
            print("Label will auto-advance to ConfirmationScreen when clicked")
            print("="*60 + "\n")
    ...
    def _try_next_discogs_release(self):
        ...
        print(f"")
        print(f"[RELOAD] AUTO-ADVANCING to next Discogs release (attempt {self.auto_advance_attempts}/{self.max_auto_advance_attempts})...")
        print(f"")
    ...
    def _update_with_current_discogs_result(self):
        ...
        # v0.00.44: Reset verification flags when starting new search
        # This allows auto-jump to trigger again for the next verified label
        self.verification_passed = False
        self.structural_valid = False
        self.ocr_valid = False
        ...
        def fetch_worker():
            ...
            try:
                ...
                # v0.00.24: Use retry wrapper with exponential backoff
                release = discogs_api_call_with_retry(d.release, result.id)
                if release is None:
                    raise Exception("Failed to fetch release after retries")
        ...
        if self.fetch_thread and not self.fetch_thread.is_alive():
            ...
            if release:
                ...
                # v0.00.10: If we have label images and we're in auto-advance mode, generate preview automatically
                    ...
                    print(f"[RELOAD] Auto-generating preview for new release...")
                    self._generate_adaptor_preview()
            else:
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.thumb_rect.collidepoint(event.pos) and self.label_image_surface:
                ...
                return "view_full_screen_image", data_bundle
            # v0.00.33: Click on adaptor preview thumbnail
            # If verified, jump directly to ConfirmationScreen; otherwise go to FinalRecordViewer
            if self.adaptor_preview_rect.collidepoint(event.pos) and self.adaptor_preview_surface:
                data_bundle = {
                    ...
                    'discogs_genres': self.genres,
                    'filename': self.filename,
                    'filename_artist': self.artist,  # v0.00.31: For automated verification
                    'filename_title': self.title     # v0.00.31: For automated verification
                }

                # v0.00.34: Debug - show verification status
                print(f"\n[DEBUG] Adaptor preview clicked:")
                print(f"  verification_passed = {self.verification_passed}")
                print(f"  structural_valid = {self.structural_valid}")
                print(f"  ocr_valid = {self.ocr_valid}")

                # v0.00.33: If verified, use OCR-verified artist/title and jump to ConfirmationScreen
                if self.verification_passed:
                    print("\n[OK] Label VERIFIED - Jumping directly to ConfirmationScreen")
                    data_bundle['ocr_verified_artist'] = self.artist  # Use filename (verified on label)
                    data_bundle['ocr_verified_title'] = self.title    # Use filename (verified on label)

                    # Load final_record.png surface for ConfirmationScreen
                    try:
                        final_record_surface = pygame.image.load('final_record.png')
                    except Exception as e:
                        print(f"Error loading final_record.png: {e}")
                        final_record_surface = None

                    confirmation_data = {
                        'label_image': final_record_surface,
                        'data_bundle': data_bundle
                    }
                    return "show_confirmation", confirmation_data
                else:
                    # Not verified - go to FinalRecordViewer for manual review
                    print("\n[!] Label NOT verified - Opening FinalRecordViewer for manual review")
...
class DetailsViewer:
    def __init__(self, screen, result):
        ...
        # v0.00.24: Use retry wrapper with exponential backoff
        self.full_release = discogs_api_call_with_retry(d.release, self.result.id)
        self.image_surface = self._load_image() if self.full_release else None
        self.song_length, self.genres = self._get_song_length_and_genres() if self.full_release else (None, [])
...
def detect_circular_label(image_bytes):
    ...
    """
    try:
        # v0.00.25: LRU cache lookup by image hash
        img_hash = hashlib.md5(image_bytes).hexdigest()
        cached_result = cache_get(CIRCLE_DETECTION_CACHE, img_hash)
        ...
        if img is None:
            result = (False, None)
            cache_put(CIRCLE_DETECTION_CACHE, img_hash, result)
            return result
        ...
        # v0.00.30: RELAXED parameters to match extract_label() flexibility
        # Pre-filter should be INCLUSIVE - let OCR validation (70% threshold) do the filtering
        # Try multiple strategies to catch more valid labels

        strategies = [
            # Strategy 1: Standard detection
            {'image': gray_filtered, 'param1': 100, 'param2': 85, 'minR': 0.20, 'maxR': 0.50},
            # Strategy 2: More sensitive
            {'image': gray_filtered, 'param1': 100, 'param2': 70, 'minR': 0.18, 'maxR': 0.52},
            # Strategy 3: Wider radius range with median blur
            {'image': gray_median, 'param1': 100, 'param2': 75, 'minR': 0.15, 'maxR': 0.55},
            # Strategy 4: Very sensitive for difficult cases
            {'image': gray_filtered, 'param1': 80, 'param2': 60, 'minR': 0.15, 'maxR': 0.55}
        ]
        ...
        for strategy in strategies:
            circles = cv2.HoughCircles(
                ...
                minRadius=int(rows * strategy['minR']),
                maxRadius=int(rows * strategy['maxR'])
            )
            ...
            if circles is not None:
                for circle in circles[0]:
                    ...
                    # v0.00.30: Relaxed to 35% offset (was 25%, extract_label uses 30%)
                    if center_offset_x < 0.35 and center_offset_y < 0.35:
                        # Check if radius is in valid 45rpm range
                        radius_ratio = radius / rows
                        # v0.00.30: Relaxed to 0.15-0.55 (was 0.20-0.50)
                        if 0.15 <= radius_ratio <= 0.55:
                            # v0.00.25: Return circle data for reuse with LRU caching
    ...
    def download_single_image(idx, img_info):
        ...
        try:
            ...
            if artist_name and song_title:
                ...
                if not ocr_match:
                    print(f"  Image {idx+1}/{len(release.images)}: OCR text mismatch, skipping [X]")
                    return None
                else:
                    print(f"  Image {idx+1}/{len(release.images)}: OCR match found [OK]")

            # Check if image contains circular patterns (45rpm label)
            is_label, circle_data = detect_circular_label(img_bytes)

            if is_label:
                print(f"  Image {idx+1}/{len(release.images)}: LABEL DETECTED [OK]")
...
def preprocess_label_for_ocr(img, debug=False):
    """
    v0.00.26: Enhanced preprocessing for vinyl record label OCR.
    Adds adaptive thresholding and optional upscaling.
    """
    # 1. Resize if too small (Tesseract needs 300+ DPI equivalent)
    height, width = img.shape[:2] if len(img.shape) == 2 else img.shape[:2]
    if max(height, width) < 600:
        scale = 600 / max(height, width)
        img = cv2.resize(img, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
        if debug:
            print(f"OCR preprocessing: Upscaled image to {int(width*scale)}x{int(height*scale)}")
    ...
    # 5. Adaptive thresholding (NEW - converts to binary black/white)
    ...
    if debug:
        print(f"OCR preprocessing: Applied CLAHE + denoise + adaptive threshold")
...
def extract_text_from_image_bytes(image_bytes, debug=False):
    ...
    try:
        # v0.00.25: LRU cache lookup by image hash
        img_hash = hashlib.md5(image_bytes).hexdigest()
        cached_result = cache_get(OCR_CACHE, img_hash)
        if cached_result is not None:
            if debug:
        ...
        # v0.00.26: Use enhanced preprocessing
        preprocessed = preprocess_label_for_ocr(img, debug=debug)

        # v0.00.26: Use PSM 11 (sparse text) and OEM 1 (LSTM)
        text = pytesseract.image_to_string(preprocessed, config='--psm 11 --oem 1')
        ...
        # v0.00.25: Store in LRU cache
        cache_put(OCR_CACHE, img_hash, text)
...
def extract_text_from_label(image_path, debug=False):
    """
    v0.00.25: Extract text from label image using OCR with LRU caching.
    Returns extracted text as a string, or empty string if OCR fails/unavailable.
    """
    if debug:
...
def check_ocr_text_match(extracted_text, artist_name, song_title, debug=False):
    """
    v0.00.26: Improved OCR text matching using rapidfuzz.
    Uses token_set_ratio and partial_ratio with higher thresholds.
    Returns True if both artist AND title match with sufficient confidence.
    """
    ...
    # v0.00.26: Use rapidfuzz for better fuzzy matching
    # token_set_ratio handles word reordering and extra words (ideal for OCR)
    artist_tsr = fuzz.token_set_ratio(artist_clean, text_clean)
    title_tsr = fuzz.token_set_ratio(title_clean, text_clean)

    # partial_ratio handles substring matching
    artist_pr = fuzz.partial_ratio(artist_clean, text_clean)
    title_pr = fuzz.partial_ratio(title_clean, text_clean)

    # Take the best score from either method
    artist_score = max(artist_tsr, artist_pr)
    title_score = max(title_tsr, title_pr)

    # v0.00.26: Raised thresholds from 50 to 70 (rapidfuzz uses 0-100 scale)
    artist_threshold = 70
    title_threshold = 70

    passed = (artist_score >= artist_threshold) and (title_score >= title_threshold)
    ...
    if debug:
        ...
        print(f"  Artist token_set_ratio: {artist_tsr}")
        print(f"  Artist partial_ratio:   {artist_pr}")
        print(f"  Title token_set_ratio:  {title_tsr}")
        print(f"  Title partial_ratio:    {title_pr}")
        print(f"  Artist best score: {artist_score} (threshold: {artist_threshold})")
        print(f"  Title best score:  {title_score} (threshold: {title_threshold})")
        print(f"")
        if passed:
            print(f"[OK] OCR MATCH CHECK: PASSED (both artist AND title found)")
        else:
            if artist_score < artist_threshold and title_score < title_threshold:
                print(f"[X] OCR MATCH CHECK: FAILED (neither artist nor title found)")
            elif artist_score < artist_threshold:
                print(f"[X] OCR MATCH CHECK: FAILED (artist not found, only title)")
            else:
                print(f"[X] OCR MATCH CHECK: FAILED (title not found, only artist)")
        print(f"")
...
def validate_label_text(image_path, artist_name, song_title, debug=False):
    ...
    try:
        ...
        # token_set_ratio handles word reordering and extra words
        artist_tsr = fuzz.token_set_ratio(artist_clean, text_clean)
        title_tsr = fuzz.token_set_ratio(title_clean, text_clean)
        ...
        # v0.00.26: Raised thresholds from 50 to 70
        artist_threshold = 70
        title_threshold = 70
        ...
        if debug:
            ...
            print(f"")
            if passed:
                print(f"[OK] OCR text validation: PASSED (both artist AND title found)")
            else:
                if artist_score < artist_threshold and title_score < title_threshold:
                    print(f"[X] OCR text validation: FAILED (neither artist nor title found)")
                elif artist_score < artist_threshold:
                    print(f"[X] OCR text validation: FAILED (artist not found, only title)")
                else:
                    print(f"[X] OCR text validation: FAILED (title not found, only artist)")
            print(f"")
...
class FinalRecordViewer:
    ...
    def __init__(self, screen, data_bundle):
        ...
        self.font_verification = pygame.font.Font(None, 48)  # v0.00.31: Large font for verification status
        ...
        # v0.00.31: Automated verification - check if final image is valid 45rpm label with correct artist/title
        self.verification_passed = False
        self.verification_message = "CHECKING..."
        self.structural_valid = False
        self.ocr_valid = False
        ...
        if self.final_record_surface:
            ...
            # Step 1: Structural validation (checks if final_record.png has proper 45rpm structure)
            try:
                self.structural_valid = validate_final_record('final_record.png', debug=True)
                if self.structural_valid:
                    print("[OK] Structural validation: PASSED (valid 45rpm label structure)")
                else:
                    print("[X] Structural validation: FAILED (malformed label/adaptor)")
            except Exception as e:
                print(f"[X] Structural validation: ERROR - {e}")
                self.structural_valid = False

            # Step 2: OCR validation (checks if artist/title match filename)
            try:
                filename_artist = data_bundle.get('filename_artist', '')
                filename_title = data_bundle.get('filename_title', '')

                if filename_artist and filename_title:
                    self.ocr_valid = validate_label_text('new_cutout_label.png', filename_artist, filename_title, debug=True)
                    if self.ocr_valid:
                        print(f"[OK] OCR validation: PASSED ('{filename_artist}' - '{filename_title}' found on label)")
                    else:
                        print(f"[X] OCR validation: FAILED (artist/title not found or below 70% threshold)")
                else:
                    print("[X] OCR validation: SKIPPED (missing filename artist/title)")
                    self.ocr_valid = False
            except Exception as e:
                print(f"[X] OCR validation: ERROR - {e}")
                self.ocr_valid = False

            # Final verdict
            self.verification_passed = self.structural_valid and self.ocr_valid

            if self.verification_passed:
                self.verification_message = "VERIFIED [OK]"
                print("\n" + "="*60)
                print("FINAL VERDICT: VERIFIED [OK]")
                print("This label passed both structural and OCR validation")
                print("="*60 + "\n")

                # v0.00.32: Auto-check "Use This Label" when verified
                self.use_label_checkbox.checked = True
                print("[OK] Auto-checked 'Use This Label' (label is verified)")

                # v0.00.32: Store OCR-verified artist/title to use instead of Discogs data
                self.data_bundle['ocr_verified_artist'] = filename_artist
                self.data_bundle['ocr_verified_title'] = filename_title
                print(f"[OK] Will use OCR-verified data: '{filename_artist}' - '{filename_title}'")
            else:
                if not self.structural_valid and not self.ocr_valid:
                    self.verification_message = "CHECK MANUALLY [!]"
                    print("\n" + "="*60)
                    print("FINAL VERDICT: CHECK MANUALLY [!]")
                    print("Failed both structural and OCR validation")
                    print("="*60 + "\n")
                elif not self.structural_valid:
                    self.verification_message = "STRUCTURE FAILED [!]"
                    print("\n" + "="*60)
                    print("FINAL VERDICT: STRUCTURE FAILED [!]")
                    print("Label structure is malformed")
                    print("="*60 + "\n")
                else:  # OCR failed
                    self.verification_message = "OCR MISMATCH [!]"
                    print("\n" + "="*60)
                    print("FINAL VERDICT: OCR MISMATCH [!]")
                    print("Artist/title not found on label or below 70% threshold")
                    print("="*60 + "\n")
    ...
    def draw(self):
        ...
        # v0.00.31: Display verification status banner
        if self.final_record_surface:
            ...
            if self.verification_passed:
                ...
                banner_color = (200, 150, 0)  # Yellow/Orange
                text_color = (0, 0, 0)  # Black for better contrast

            # Draw banner background
            banner_rect = pygame.Rect(0, 90, self.screen.get_width(), 60)
            pygame.draw.rect(self.screen, banner_color, banner_rect)

            # Draw verification message
            verification_surf = self.font_verification.render(self.verification_message, True, text_color)
            verification_x = (self.screen.get_width() - verification_surf.get_width()) // 2
            self.screen.blit(verification_surf, (verification_x, 105))

            # Draw detailed status (small text below main message)
            status_font = pygame.font.Font(None, 20)
            if self.structural_valid and self.ocr_valid:
                detail_text = "Structural: [OK]  |  OCR Match: [OK]"
            elif self.structural_valid:
                detail_text = "Structural: [OK]  |  OCR Match: [X]"
            elif self.ocr_valid:
                detail_text = "Structural: [X]  |  OCR Match: [OK]"
            else:
                detail_text = "Structural: [X]  |  OCR Match: [X]"

            detail_surf = status_font.render(detail_text, True, text_color)
            detail_x = (self.screen.get_width() - detail_surf.get_width()) // 2
            self.screen.blit(detail_surf, (detail_x, 132))

        # Display final_record.png at its original size (420x420) centered
        if self.final_record_surface:
            # Center the 420x420 image on screen (lower to make room for banner)
            screen_center_x = self.screen.get_width() // 2
            image_x = screen_center_x - 210  # 420/2 = 210
            image_y = 160  # v0.00.31: Moved down from 100 to make room for banner

            self.screen.blit(self.final_record_surface, (image_x, image_y))

            # v0.00.06: Display metadata below image
            # v0.00.31: Adjusted for new image position
...
class ID3EditScreen:
    def __init__(self, screen, data_bundle):
        ...
        self.artist_input = EnhancedInputBox(x_input, y_start - 5, input_width, 30, self.artist)

        self.title_checkbox = ColoredCheckbox(x_checkbox, y_start + 30, 20, 20)
        self.title_input = EnhancedInputBox(x_input, y_start + 30 - 5, input_width, 30, self.title)

        self.album_checkbox = ColoredCheckbox(x_checkbox, y_start + 60, 20, 20)
        self.album_input = EnhancedInputBox(x_input, y_start + 60 - 5, input_width, 30, self.album)

        self.genre_checkbox = ColoredCheckbox(x_checkbox, y_start + 90, 20, 20)
        self.genre_input = EnhancedInputBox(x_input, y_start + 90 - 5, input_width, 30, self.genre)

        self.year_checkbox = ColoredCheckbox(x_checkbox, y_start + 120, 20, 20)
        self.year_input = EnhancedInputBox(x_input, y_start + 120 - 5, input_width, 30, self.year)

        self.comment_checkbox = ColoredCheckbox(x_checkbox, y_start + 150, 20, 20)
        self.comment_input = EnhancedInputBox(x_input, y_start + 150 - 5, input_width, 30, self.comment)
...
class ConfirmationScreen:
    def __init__(self, screen, confirmation_data, show_image=True):
        ...
        self.title_input = EnhancedInputBox(x_input, y_start - 5, input_width, 30, self.title)

        self.artist_checkbox = ColoredCheckbox(x_checkbox, y_start + 30, 20, 20)
        self.artist_input = EnhancedInputBox(x_input, y_start + 30 - 5, input_width, 30, self.artist)

        self.year_checkbox = ColoredCheckbox(x_checkbox, y_start + 60, 20, 20)
        self.year_input = EnhancedInputBox(x_input, y_start + 60 - 5, input_width, 30, self.year)

        self.time_checkbox = ColoredCheckbox(x_checkbox, y_start + 90, 20, 20)
        self.time_input = EnhancedInputBox(x_input, y_start + 90 - 5, input_width, 30, self.time)

        self.genre_checkbox = ColoredCheckbox(x_checkbox, y_start + 120, 20, 20)
        self.genre_input = EnhancedInputBox(x_input, y_start + 120 - 5, input_width, 30, self.genres)
    ...
    def _get_title(self):
        """
        v0.00.32: Use OCR-verified title if available (from verified label), otherwise Discogs title
        """
        # Prefer OCR-verified title (actual text from label that passed validation)
        if 'ocr_verified_title' in self.data_bundle:
            title = self.data_bundle['ocr_verified_title']
        else:
    ...
    def _get_artist(self):
        """
        v0.00.32: Use OCR-verified artist if available (from verified label), otherwise Discogs artist
        Sanitized, with 'The ' removed from start (case-insensitive)
        """
        # Prefer OCR-verified artist (actual text from label that passed validation)
        if 'ocr_verified_artist' in self.data_bundle:
            artist = self.data_bundle['ocr_verified_artist']
        else:
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key in (pygame.K_RETURN, pygame.K_ESCAPE):
                if focused_widget == self.back_button or event.key == pygame.K_ESCAPE:
                    # v0.00.41: Always return to file_display (to continue searching Discogs)
                    return "back_to_file_display"
                elif focused_widget == self.write_to_file_button:
                    # Show confirmation buttons
                    self.show_confirmation_buttons = True
...
class SearchOptionsScreen:
    def __init__(self, screen, music_files):
        ...
        input_x = 300
        input_width = 600
        self.artist_input = EnhancedInputBox(input_x, y_start + 100, input_width, 30, "Enter Artist Name")
        self.title_input = EnhancedInputBox(input_x, y_start + 170, input_width, 30, "Enter Title")
        self.genre_input = EnhancedInputBox(input_x, y_start + 310, input_width, 30, "Enter Genre")
...
def draw_status_bar(screen, file_index, total_files, status_text, prefetch_count=0):
    """
    v0.00.25: Draw a persistent status bar at the bottom of the screen.
    Shows file counter, processing status, and prefetch indicator.
    """
    bar_height = 30
    ...
    # Draw top border
    pygame.draw.line(screen, (80, 80, 80), (0, bar_rect.y), (screen.get_width(), bar_rect.y), 1)
    ...
    # File counter
    if total_files > 0:
        file_text = f"File {file_index + 1} of {total_files}"
    else:
        file_text = "No files"

    # Combine with status
    full_text = f"{file_text} | {status_text}"

    # Add prefetch indicator if applicable
    if prefetch_count > 0:
        full_text += f" | Prefetched: {prefetch_count}"

    # Render and display
    text_surf = font_status.render(full_text, True, (200, 200, 200))
    screen.blit(text_surf, (10, bar_rect.y + 6))

    # Add cache stats on the right side
    global CACHE_HITS, CACHE_MISSES
    cache_text = f"Cache: {CACHE_HITS}H/{CACHE_MISSES}M"
    cache_surf = font_status.render(cache_text, True, (120, 120, 120))
    screen.blit(cache_surf, (screen.get_width() - cache_surf.get_width() - 10, bar_rect.y + 6))
    ...
    for music_file in music_files:
        try:
            ...
            if comment and genre_lower in comment.lower():
                ...
                print(f"  [OK] Found: {music_file} (Comment: {comment})")
...
def main():
    ...
    # v0.00.25: Initialize prefetch manager with config values
    prefetch_count_cfg = getattr(music_file_cleaner_config, 'PREFETCH_COUNT', 2)
    prefetch_workers_cfg = getattr(music_file_cleaner_config, 'PREFETCH_WORKERS', 2)
    PREFETCH_MANAGER = PrefetchManager(max_prefetch=prefetch_count_cfg, max_workers=prefetch_workers_cfg)
    current_status = "Idle"  # v0.00.25: Track processing status

    # v0.00.25: Apply config cache size
    global MAX_CACHE_SIZE
    MAX_CACHE_SIZE = getattr(music_file_cleaner_config, 'CACHE_SIZE', 500)

    # State variables
    search_options_screen = None
    genre_selection_screen = None  # v0.00.14
    file_display = None
    file_display_prev_verified = False  # v0.00.42: Track previous verification state for auto-jump
    ...
    while not done:
        ...
        for event in pygame.event.get():
            ...
            if app_state == "search_options" and search_options_screen:
                ...
                if action == "continue":
                    ...
                    # v0.00.35: Handle auto-jump from verified label in FileDisplay
                    print("[DEBUG] show_confirmation handler triggered in main event loop")
                    print(f"[DEBUG] Creating ConfirmationScreen with data keys: {data.keys() if isinstance(data, dict) else 'NOT A DICT'}")
                    print(f"[DEBUG] Changing app_state from 'file_display' to 'confirmation'")
                    confirmation_screen = ConfirmationScreen(screen, data)
                    app_state = "confirmation"
                    print(f"[DEBUG] ConfirmationScreen created, app_state = {app_state}")
                ...
                if result:
                    ...
                    if action == "back_to_result":
                        ...
                        # v0.00.44: Auto-continue searching next Discogs result
                        app_state = "file_display"
                        confirmation_screen = None
                        if file_display:
                            file_display.auto_advance_attempts = 0  # Reset counter
                            file_display._update_with_current_discogs_result()  # Cycle to next result
        ...
        if app_state == "file_display" and file_display:
            ...
            # v0.00.42: Check for auto-jump AFTER update() (when background thread completes)
            # Only jump when verification TRANSITIONS from False to True (prevents multiple jumps)
            if file_display.verification_passed and not file_display_prev_verified:
                file_display_prev_verified = True  # Mark current state

                print("\n" + "="*60)
                print("AUTO-JUMP: Label verified - jumping to ConfirmationScreen")
                print("="*60 + "\n")
                ...
                # Create data bundle for ConfirmationScreen
                data_bundle = {
                    'mutagen_tags': file_display.mutagen_tags,
                    'discogs_artist': file_display.discogs_artist,
                    'discogs_title': file_display.discogs_title,
                    'discogs_year': file_display.year,
                    'discogs_genres': file_display.genres,
                    'filename': file_display.filename,
                    'ocr_verified_artist': artist,  # Use filename (verified on label)
                    'ocr_verified_title': title     # Use filename (verified on label)
                }

                # Load final_record.png for display
                try:
                    final_record_surface = pygame.image.load('final_record.png')
                except Exception as e:
                    print(f"Error loading final_record.png: {e}")
                ...
                confirmation_screen = ConfirmationScreen(screen, confirmation_data)
                app_state = "confirmation"
                current_status = "Ready"
            elif not file_display.verification_passed and file_display_prev_verified:
                # v0.00.42: Reset tracking when verification goes back to False (new Discogs result loading)
                file_display_prev_verified = False
        ...
        if app_state == "search_options":
            ...
            if search_thread is None:
                if current_file_index >= len(music_files):
                        ...
                        if should_query_discogs:
                            ...
                            if results:
                                try:
                                    ...
                                    release = discogs_api_call_with_retry(d.release, results[0].id)
                                    if release is None:
                                        print("Failed to fetch release after retries, skipping...")
                                    else:
                                        fetched_data['full_release'] = release
                    ...
                    if search_config and search_config['mode'] == 'from_discogs_list':
                        ...
                        # v0.00.25: Check prefetch cache before starting search thread
                        prefetched = PREFETCH_MANAGER.get_cached(current_file) if PREFETCH_MANAGER else None
                        if prefetched:
                            print(f"[Prefetch] Cache hit for {current_file} - skipping search thread")
                            search_result['data'] = prefetched
                            # Start a no-op thread so the thread-completion check works
                            search_thread = threading.Thread(target=lambda: None)
                            search_thread.start()
                        else:
                            search_result.clear()
            ...
            # v0.00.25: Show searching status with status bar
            current_status = "Searching Discogs..."
            searching_text = FONT.render("Searching...", True, (255, 255, 255))
            text_rect = searching_text.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2 - 20))
            screen.blit(searching_text, text_rect)

            # v0.00.25: Draw status bar
            prefetch_count = PREFETCH_MANAGER.get_prefetch_count() if PREFETCH_MANAGER else 0
            draw_status_bar(screen, current_file_index, len(music_files), current_status, prefetch_count)
            ...
            if search_thread is not None and not search_thread.is_alive():
                ...
                if fetched_data and fetched_data.get('discogs_results'):
                    ...
                    # v0.00.40: Start in file_display state - auto-jump check happens after update()
                    app_state = "file_display"
                    current_status = "Ready"

                    # v0.00.25: Start prefetching next files
                    if PREFETCH_MANAGER:
                        PREFETCH_MANAGER.prefetch(music_files, current_file_index, search_config)
                else:
            ...
            if file_display:
                ...
                # v0.00.25: Draw status bar on file display
                current_status = "Ready - Review file"
                prefetch_count = PREFETCH_MANAGER.get_prefetch_count() if PREFETCH_MANAGER else 0
                draw_status_bar(screen, current_file_index, len(music_files), current_status, prefetch_count)
    ...
    # v0.00.25: Shutdown prefetch manager
    if PREFETCH_MANAGER:
        PREFETCH_MANAGER.shutdown()
        print(f"[Cache Stats] Hits: {CACHE_HITS}, Misses: {CACHE_MISSES}")
