...
class ID3EditScreen:
    def __init__(self, screen, data_bundle):
        ...
        # Create InputBoxes for each field
        y_start = 380
        x_label = 120
        x_input = 250
        input_width = 700
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.show_confirmation_buttons:
                if self.are_you_sure_button.rect.collidepoint(event.pos):
                    # Get final values from input boxes
                    final_artist = self.artist_input.text
                    final_title = self.title_input.text
                    final_album = self.album_input.text
                    final_genre = self.genre_input.text
                    final_year = self.year_input.text
                    final_comment = self.comment_input.text
    ...
    def draw(self):
        ...
        labels = ["Artist:", "Title:", "Album:", "Genre:", "Year:", "Comment:"]
        for i, (label_text, input_box) in enumerate(zip(labels, self.input_boxes)):
            label_surf = self.font_label.render(label_text, True, (200, 200, 200))
            self.screen.blit(label_surf, (x_label, y_start + i * 30))
        ...
        if self.show_confirmation_buttons:
            self.are_you_sure_button.draw(self.screen)
