...
Version 0.91.20 - Random Playlist Priority System

This version changes how the random playlist is built. Filtered songs (matching
genre/year) are shuffled and placed at the front of the playlist for priority
playback. All remaining songs (excluding 'norandom') are shuffled and placed at
the back so every song in the library eventually gets played. When the entire
list is exhausted, it regenerates with the same priority ordering. Paid songs
are removed from the random list when played to prevent double plays.

Version 0.91.20 Changes:
- CHANGED generate_random_playlist() to use two-tier priority system
- ADDED filtered songs (genre/year match) shuffled to front of random list
- ADDED remaining non-norandom songs shuffled to back of random list
- CHANGED random playback to consume songs (not rotate) and regenerate when exhausted
- ADDED removal of paid songs from random list to prevent double plays
...
class PlaybackEngine:
    ...
    def generate_random_playlist(self) -> None:
        """Generate random playlist with two-tier priority: filtered songs first, then everything else.

        Songs tagged 'norandom' are always excluded. Filtered songs (matching genre/year)
        are shuffled and placed at the front. All remaining eligible songs are shuffled
        and placed at the back so every song eventually gets played."""
        # Load current genre filter settings
        self.load_genre_flags()

        priority_songs = []   # Songs matching filters (front of list)
        remaining_songs = []  # All other eligible songs (back of list)

        has_filters = not (self.genre0 == "null" and self.genre1 == "null" and
                          self.genre2 == "null" and self.genre3 == "null")
        ...
        for index, song in enumerate(self.song_list):
            ...
            # Check if song matches year range filter
            matches_year = True
            ...
            # Check if song matches genre filters
            matches_genre = False
            if has_filters:
            ...
            # Determine if song is a priority (matches active filters) or remaining
            if has_filters or self.year_range_enabled:
                if (matches_year if self.year_range_enabled else True) and \
                   (matches_genre if has_filters else True):
                    priority_songs.append(index)
                else:
                    remaining_songs.append(index)
            else:
                # No filters active - all songs go into one pool
                remaining_songs.append(index)

        # Shuffle each group independently
        random.shuffle(priority_songs)
        random.shuffle(remaining_songs)

        # Priority songs at front, remaining at back
        self.random_playlist = deque(priority_songs + remaining_songs)
        print(f"[RANDOM PLAYLIST] Generated {len(self.random_playlist)} songs ({len(priority_songs)} priority, {len(remaining_songs)} remaining)")
        ...
        for _retry in range(50):
            ...
            if len(self.paid_playlist) > 0:
                ...
                if song_index < len(self.song_list):
                    ...
                    # Remove this song from random list so it doesn't play twice
                    if song_index in self.random_playlist:
                        self.random_playlist.remove(song_index)
                        print(f"Removed paid song {song_index} from random playlist")
            ...
            # Priority 2: Play random songs if no paid songs AND random music is enabled
                ...
                if song_index < len(self.song_list):
                    ...
                    # Remove from random playlist (consumed, not rotated)
