                ...
                if event == "-NEXT-" or event == "-PREV-" or event == "--CLEAR--" or event == '--EXIT--':
                    ...
                    if search_flag == "title":
                        if event == "--result_one--" or event == "--result_two--" or event == "--result_three--" or event == "--result_four--" or event == "--result_five--":
                            ...
                            # Extract song number directly from search results tuple (instant - no search!)
                            song_selected_number = search_results[result_index][0]

                            # Find song in MusicMasterSongList by number to get array index for display
                            for i in range(len(MusicMasterSongList)):
                                if MusicMasterSongList[i]['number'] == song_selected_number:
                                    # Song found
                    ...
                    if search_flag == "artist":
                        if event == "--result_one--" or event == "--result_two--" or event == "--result_three--" or event == "--result_four--" or event == "--result_five--":
                            ...
                            # Extract song number directly from search results tuple (instant - no search!)
                            song_selected_number = search_results[result_index][0]
                            artist_name = search_results[result_index][1]  # Get artist name for debug printing

                            print('artist looking for is: ' + str(artist_name))
                            print('artist match found')
                            print("Title Selected is number: " + str(song_selected_number))
                            print("Artist Selected is: " + str(artist_name))
                    ...
                    search_results = []
                    # Strip leading/trailing spaces from search term for better matching
                    search_term = keys_entered.strip()
                    print(f"[SEARCH DEBUG] keys_entered='{keys_entered}' search_term='{search_term}' len={len(search_term)}")
                    # Code to search for song title based on keys entered
                    # Only search if there's actual text after stripping
                    if search_term and search_flag == "title":
                        match_count = 0  # Debug counter
                        for i, item in enumerate(MusicMasterSongList):
                            find_list_search = MusicMasterSongList[i]['title']
                            match = find_list_search.lower().find(search_term.lower())
                            # Debug: show first 3 comparisons
                            if i < 3:
                                print(f"[SEARCH DEBUG] Comparing '{find_list_search}' vs '{search_term}' -> match={match}")
                            if match == 0:  # ensures match is from left of string
                                match_count += 1
                                # Store tuple: (song_number, "Artist - Title") for instant lookup on selection
                                search_results.append((MusicMasterSongList[i]['number'], str(MusicMasterSongList[i]['artist']) + " - " + str(MusicMasterSongList[i]['title'])))
                        print(f"[SEARCH DEBUG] Title search found {len(search_results)} results")
                        if search_results:
                            print(f"[SEARCH DEBUG] First match: {search_results[0]}")
                    # Code to search for artist based on keys entered
                    # Only search if there's actual text after stripping
                    if search_term and search_flag == "artist":
                        seen_artists = set()  # Track artists already added to avoid duplicates
                        print(keys_entered)
                        for i, item in enumerate(MusicMasterSongList):
                            artist_name = MusicMasterSongList[i]['artist']
                            match = artist_name.lower().find(search_term.lower())
                            if match == 0 and artist_name not in seen_artists:  # ensures match is from left of string and not duplicate
                                # Store tuple: (song_number, "Artist") for instant lookup on selection
                                search_results.append((MusicMasterSongList[i]['number'], str(artist_name)))
                                seen_artists.add(artist_name)
                    # Code to update search results on search window
                    print(f"[UPDATE DEBUG] About to update buttons, search_results length = {len(search_results)}")
                    if len(search_results) >= 5:
                        print("[UPDATE DEBUG] Running visibility code for >= 5 results")
                    ...
                    if len(search_results) == 0:
                        ...
                        search_window["--result_one--"].update(search_results[0][1], visible=True, disabled=False)
                        search_window["--result_two--"].update("", visible=False)
                        search_window["--result_three--"].update("", visible=False)
                        search_window["--result_four--"].update("", visible=False)
                        search_window["--result_five--"].update("", visible=False)
                        ...
                        search_window["--result_one--"].update(search_results[0][1], visible=True, disabled=False)
                        search_window["--result_two--"].update(search_results[1][1], visible=True)
                        search_window["--result_three--"].update("", visible=False)
                        search_window["--result_four--"].update("", visible=False)
                        search_window["--result_five--"].update("", visible=False)
                    elif len(search_results) == 3:
                        search_window["--result_one--"].update(search_results[0][1], visible=True, disabled=False)
                        search_window["--result_two--"].update(search_results[1][1], visible=True)
                        search_window["--result_three--"].update(search_results[2][1], visible=True)
                        search_window["--result_four--"].update("", visible=False)
                        search_window["--result_five--"].update("", visible=False)
                    elif len(search_results) == 4:
                        search_window["--result_one--"].update(search_results[0][1], visible=True, disabled=False)
                        search_window["--result_two--"].update(search_results[1][1], visible=True)
                        search_window["--result_three--"].update(search_results[2][1], visible=True)
                        search_window["--result_four--"].update(search_results[3][1], visible=True)
                        search_window["--result_five--"].update("", visible=False)
                        ...
                        search_window["--result_three--"].update(search_results[2][1], visible=True)
                        search_window["--result_four--"].update(search_results[3][1], visible=True)
                        search_window["--result_five--"].update(search_results[4][1], visible=True)
                    elif len(search_results) > 5:
                        print(f"[UPDATE DEBUG] Running text update for > 5 results, first result: {search_results[0][1]}")
                        search_window["--result_one--"].update(search_results[0][1], visible=True, disabled=False)
                        search_window["--result_two--"].update(search_results[1][1], visible=True)
                        search_window["--result_three--"].update(search_results[2][1], visible=True)
                        search_window["--result_four--"].update(search_results[3][1], visible=True)
                        search_window["--result_five--"].update(search_results[4][1], visible=True)
                        print("[UPDATE DEBUG] Text update complete")
