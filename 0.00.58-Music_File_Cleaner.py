...
class ConfirmationScreen:
    ...
    def _get_artist(self):
        """Always use Discogs artist, sanitized, with 'The ' removed from start"""
        artist = self.data_bundle['discogs_artist']
        sanitized = self._sanitize_text(artist)
        # Remove "The " from the start of artist name if present
        if sanitized and sanitized.startswith('The '):
            sanitized = sanitized[4:]  # Remove "The " (4 characters)
                ...
                if current_file_index >= len(music_files):
                    ...
                    def search_worker(music_file):
                        ...
                        try:
                            ...
                            # REMOVED: Skip logic for files with "image" in comment (v0.00.52)
                            # Now processes all files regardless of comment content
