...
TESTING VERSION - Only processes first 5 records
"""
...
TIMEOUT = 30
LIMIT_RECORDS = 5  # TEST: Only process first 5 records
...
def main():
    """Main function"""
    print("=" * 70)
    print("Archive.org Song Name List Generator v0.00.00")
    print("=" * 70)
    print(f"TESTING MODE: Processing only first {LIMIT_RECORDS} records")
    print("=" * 70)
    print()
    ...
    limited_songs = all_songs[:LIMIT_RECORDS]

    print(f"\nProcessing first {len(limited_songs)} records:\n")
    print("=" * 70)
    for i, (orig, mp3, swapped) in enumerate(limited_songs, 1):
        print(f"{i}.")
    ...
    print("=" * 70)
    generate_output_file(limited_songs, OUTPUT_FILE)

    print("\n" + "=" * 70)
    print("Complete!")
    print("=" * 70)
    print(f"\nReview the output file '{OUTPUT_FILE}' to verify format.")
    print(f"If satisfied, update LIMIT_RECORDS to process all {len(all_songs)} songs.")
