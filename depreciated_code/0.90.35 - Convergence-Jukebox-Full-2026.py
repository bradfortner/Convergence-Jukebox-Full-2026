...
Version 0.90.35 - Fix TinyTag Crash with Corrupt MP3 Files
...
Version 0.90.35 Changes:
- FIXED TinyTag segmentation fault crash with corrupt MP3 files
- Added BaseException catch to handle low-level crashes (not just Python exceptions)
- Added try-except wrapper around entire metadata extraction loop
- If file causes crash, it's skipped and processing continues with next file
- Progress continues even when encountering problematic MP3 files
- More defensive error handling ensures song list generation always completes

Version 0.90.34 Changes:
- FIXED pygame double initialization crash in song list generation
- Replaced threaded pygame progress bar with console-based progress display
- Console shows progress every 100 files during metadata extraction
- Avoids pygame initialization conflict (main app vs progress bar thread)
- Song list generation now works reliably for all collection sizes
- Removed MetadataProgressBar import (no longer needed)

Version 0.90.33 Changes:
- RESTORED song list generation functionality from original version (0.83.60)
...
- Shows threaded pygame progress bar during initial scan
...
- Handles corrupt/missing metadata gracefully (uses filename as fallback)
- Jukebox now works out-of-box for new users with MP3 collection
- Added imports: glob, threading, TinyTag
- Added MetadataProgressBar module (threaded pygame progress window)
- Added functions: scan_music_directory(), extract_song_metadata(), generate_song_list(), should_regenerate()
- Modified main() startup to check and regenerate song list when needed
...
def scan_music_directory(music_dir):
    """
    Scan music directory for all MP3 files.

    Args:
        music_dir (str): Path to music directory

    Returns:
        list: List of absolute file paths to all MP3 files
    """
    try:
        mp3_files = glob.glob(os.path.join(music_dir, '*.mp3'))
        print(f"[SCAN] Found {len(mp3_files)} MP3 files in {music_dir}")
        return mp3_files
    except Exception as e:
        print(f"[ERROR] Failed to scan music directory: {e}")
        return []

def extract_song_metadata(file_path, index):
    """
    Extract ID3 metadata from MP3 file using TinyTag.
    Handles corrupt files gracefully without crashing.

    Args:
        file_path (str): Path to MP3 file
        index (int): Song number/index

    Returns:
        dict: Song metadata with keys: number, location, title, artist, album, year, comment, duration
    """
    try:
        # Extract metadata using TinyTag (can cause segfaults on corrupt files)
        tag = TinyTag.get(file_path, ignore_errors=True)

        # Get metadata with fallbacks for missing data
        ...
        comment = tag.comment if tag.comment else ""
        ...
        # Catch ALL exceptions including low-level crashes
        # Fallback for corrupt/missing metadata
        filename = os.path.basename(file_path)
        print(f"[WARNING] Skipping corrupt file: {filename} ({type(e).__name__})")
        return {
            'number': index,
            'location': file_path,
            'title': filename,
...
def generate_song_list(music_dir, output_file, check_file):
    """
    Generate song list by scanning music directory and extracting metadata.
    Shows console progress during processing.

    Args:
        music_dir (str): Path to music directory
        output_file (str): Path to save MusicMasterSongList.txt
        check_file (str): Path to save MusicMasterSongListCheck.txt
    ...
    # Scan for MP3 files
    mp3_files = scan_music_directory(music_dir)
    ...
    print(f"\n[PROGRESS] Extracting metadata from {total_files} MP3 files...")
    print("[PROGRESS] This may take a few minutes for large collections...")
    ...
    try:
        ...
        for index, file_path in enumerate(mp3_files):
            ...
            if (index + 1) % 100 == 0 or index == 0:
                ...
                print(f"[PROGRESS] {index + 1}/{total_files} ({percent:.1f}%) - {os.path.basename(file_path)}")

            # Extract metadata with defensive error handling
            try:
                song_data = extract_song_metadata(file_path, index)
                song_list.append(song_data)
            except BaseException as e:
                # If extraction fails catastrophically, create fallback entry
                filename = os.path.basename(file_path)
                print(f"[ERROR] Critical error processing {filename}: {type(e).__name__}")
                song_list.append({
                    'number': index,
                    'location': file_path,
                    'title': filename,
        ...
        # Show 100% completion
        print(f"[PROGRESS] {total_files}/{total_files} (100.0%) - Complete!")

        # Save song list to file
        print(f"\n[SAVE] Writing song list to {output_file}...")
        with open(output_file, 'w') as f:
            json.dump(song_list, f, indent=2)
        print(f"[SAVE] Saved {len(song_list)} songs to {output_file}")

        # Save file count for change detection
        with open(check_file, 'w') as f:
            json.dump(len(mp3_files), f)
        print(f"[SAVE] Saved file count ({len(mp3_files)}) to {check_file}")

    except Exception as e:
        print(f"[ERROR] Failed to generate song list: {e}")
        song_list = []
...
def should_regenerate(music_dir, check_file):
    """
    Determine if song list should be regenerated.
    Compares current MP3 file count with stored count.
    ...
    """
    # If check file doesn't exist, regenerate
    if not os.path.exists(check_file):
        print("[CHECK] Song list check file not found - regeneration needed")
        return True

    # Count current MP3 files
    current_count = len(glob.glob(os.path.join(music_dir, '*.mp3')))

    # Load stored count
    try:
        with open(check_file, 'r') as f:
            stored_count = json.load(f)
    except (IOError, json.JSONDecodeError):
        print("[CHECK] Failed to read check file - regeneration needed")
        return True

    # Compare counts
    if current_count != stored_count:
        print(f"[CHECK] File count changed ({stored_count} → {current_count}) - regeneration needed")
        return True
    else:
        print(f"[CHECK] File count unchanged ({current_count} files) - using existing song list")
        return False
...
def main():
    ...
    # Load or generate song list
    song_list = []

    # Check if song list exists and if regeneration is needed
    if os.path.exists(SONG_LIST_PATH):
        # Song list exists - check if regeneration needed
        if should_regenerate(MUSIC_DIRECTORY, MUSIC_MASTER_SONG_LIST_CHECK_PATH):
            # File count changed - regenerate
            song_list = generate_song_list(MUSIC_DIRECTORY, SONG_LIST_PATH, MUSIC_MASTER_SONG_LIST_CHECK_PATH)
        else:
            # File count unchanged - load existing
            try:
                with open(SONG_LIST_PATH, 'r') as f:
                    song_list = json.load(f)
                print(f"[LOAD] Loaded {len(song_list)} songs from {SONG_LIST_PATH}")
            except (FileNotFoundError, json.JSONDecodeError) as e:
                print(f"[ERROR] Failed to load song list: {e}")
                print("[FALLBACK] Regenerating song list...")
                song_list = generate_song_list(MUSIC_DIRECTORY, SONG_LIST_PATH, MUSIC_MASTER_SONG_LIST_CHECK_PATH)
    else:
        # Song list doesn't exist - generate it
        print(f"[INIT] Song list not found - generating from {MUSIC_DIRECTORY}...")
        song_list = generate_song_list(MUSIC_DIRECTORY, SONG_LIST_PATH, MUSIC_MASTER_SONG_LIST_CHECK_PATH)

    # Verify we have songs to work with
    if not song_list:
        print("\n" + "="*60)
        print("ERROR: No songs found!")
        print(f"Please add MP3 files to the '{MUSIC_DIRECTORY}' directory")
        print("="*60)
        pygame.quit()
        sys.exit(1)
