...
Version 0.90.49 - Genre List Logging
...
def main():
    ...
    # Extract all unique comment values (genres)
    all_genres = set()
    for song in song_list:
        comment = song.get('comment', '')
        if comment:  # Only include non-empty comments
            all_genres.add(comment)

    # Sort genres alphabetically for readability
    sorted_genres = sorted(all_genres)

    # Print to console
    print(f"[GENRES] Found {len(sorted_genres)} unique genre combinations:")
    ...
    try:
        with open(LOG_FILE_PATH, 'a') as log:
            log.write(f'\n{log_date}, {log_time}, Genres Available ({len(sorted_genres)} unique combinations):\n')
            for genre in sorted_genres:
