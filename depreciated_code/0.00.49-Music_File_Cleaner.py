    ...
    def handle_event(self, event):
        ...
        if self.use_label_checkbox.checked and not was_checked_before:
            if not self.image_surface:
                ...
                resized_surface = pygame.transform.scale(self.image_surface, (350, 350))
...
class PreviewScreen:
    def __init__(self, screen, preview_data):
        ...
        # Receive the preview surface directly (already loaded from disk in ConfirmationScreen)
        self.preview_image = preview_data.get('preview_surface')
        if self.preview_image:
            print(f"PreviewScreen: Received surface: {self.preview_image.get_width()}x{self.preview_image.get_height()}")
        elif self.show_image:
            print("PreviewScreen: WARNING - No preview surface received!")
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.show_confirmation_buttons:
                if self.are_you_sure_button.rect.collidepoint(event.pos):
                    ...
                    # Create image if in image mode, then read it back immediately
                    ...
                    if self.show_image:
                        ...
                        if success:
                            print(f"File written: final_record.png ({os.path.getsize('final_record.png')} bytes)")

                            # Read the file back from disk immediately
                            print("Reading final_record.png from disk...")
                            preview_surface = pygame.image.load('final_record.png')
                            print(f"Loaded surface from disk: {preview_surface.get_width()}x{preview_surface.get_height()}")
                        else:
                            print("WARNING: Failed to create final record label.")

                    # Go to preview screen with the pygame surface
                    preview_data = {
                        'filename': self.filename,
