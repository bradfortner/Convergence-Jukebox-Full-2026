...
def main():
    ...
    # These widgets are for a different app state, but defined here for simplicity
    ...
    result_viewer = None
    app_state = "searching_music_file"  # Start in the new searching state
    ...
    while not done:
        ...
        for event in events:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    # Special case: Exit if ESC is pressed during the *initial* search
                    if app_state == "searching_music_file" and current_file_index == 0 and file_display is None:
                        done = True
                    # General ESC handling for other states
                        ...
                        result_viewer = None
                    # In file_display, ESC will now effectively do nothing unless we add a quit condition
        ...
        if app_state == "searching_music_file":
            # 1. Draw the searching screen immediately
            ...
            # 2. Perform the blocking file loading logic
            ...
            if not file_display.discogs_results:
                ...
                current_file_index += 1
                file_display = None
                # Loop back to load the next file, staying in the searching state
                continue 
            else:
                # 3. If successful, switch to the display state for the next frame
                app_state = "file_display"
