    ...
    def __init__(self, screen, preview_data):
        ...
        if self.show_image:
            ...
            if os.path.exists('final_record.png'):
                ...
                print(f"PreviewScreen: Loading final_record.png ({file_size} bytes)")
                if file_size > 0:
                    try:
                        self.preview_image = pygame.image.load('final_record.png')
                        print("PreviewScreen: Successfully loaded preview image")
                    except pygame.error as e:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.show_confirmation_buttons:
                if self.are_you_sure_button.rect.collidepoint(event.pos):
                    ...
                    if self.show_image:
                        print("Creating final record label preview...")
                        ...
                        if success:
                            ...
                            # Give filesystem time to flush the file to disk
                            import time
                            time.sleep(0.1)
                            # Verify file exists and has content
                            if os.path.exists('final_record.png') and os.path.getsize('final_record.png') > 0:
                                print(f"Verified final_record.png ({os.path.getsize('final_record.png')} bytes)")
                            else:
                                print("WARNING: final_record.png not found or empty after creation!")
                        else:
