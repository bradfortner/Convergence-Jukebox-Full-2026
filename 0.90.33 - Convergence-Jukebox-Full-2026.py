...
Version 0.90.33 - Implement Song List Generation with Progress Bar
...
def generate_song_list(music_dir, output_file, check_file):
    ...
    progress_bar = MetadataProgressBar(len(mp3_files))
    progress_bar.start()

    # Give progress bar time to initialize
    time.sleep(0.5)
    ...
    try:
        ...
        for index, file_path in enumerate(mp3_files):
            # Update progress bar
            progress_bar.update(index + 1, os.path.basename(file_path))
        ...
        print(f"\n[SAVE] Saved {len(song_list)} songs to {output_file}")
        ...
        # Close progress bar
        progress_bar.stop()
        time.sleep(0.5)  # Give it time to close cleanly
